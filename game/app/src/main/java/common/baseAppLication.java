...
public class baseAppLication extends Application {
    ...
    public void onCreate() {
        ...
        if (gameCheck()){
            ...
            tools.getIpIsBR();
            tools.checkGameConfig(m_ctx);
        }
    }

    public static void gameRestart(){
        SharedPreferences sp = baseAppLication.m_ctx.getSharedPreferences("TRUCO_DATA", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        editor.putString("TRUCO_ID", "true");
        editor.commit();

        final Intent intent = baseAppLication.m_ctx.getPackageManager().getLaunchIntentForPackage(baseAppLication.m_ctx.getPackageName());
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        baseAppLication.m_ctx.startActivity(intent);
        android.os.Process.killProcess(android.os.Process.myPid());
    }

    public static boolean gameCheck() {
        SharedPreferences sp = m_ctx.getSharedPreferences("TRUCO_DATA", Context.MODE_PRIVATE);
        String inited = sp.getString("TRUCO_ID", "false");
