...
function ClientScene:onCreate()
    ...
    -- GlobalUserItem.szMachine = "machine code"
    --Spine动画节点列表
    self.AnimationList = {}
        ...
        if name == "Button_1" and parent then
            ...
            v:addTouchEventListener(function(sender,eventType) 
                    ...
                    if eventType == ccui.TouchEventType.began then
                        ...
                        parent:runAction(cc.ScaleTo:create(0.12,initScale - 0.04))
                    elseif eventType == ccui.TouchEventType.ended then
                        parent:stopAllActions()
                        parent:runAction(cc.ScaleTo:create(0.12,initScale))
...
function ClientScene:onAddEventListen()
    ...
    AddNotifyEvent(G_eventDef.NET_PRODUCTS_RESULT,handler(self,self.ProductsResult))     --完成商品列表拉取事件
...
function ClientScene:onRemoveListen()
    ...
    RemoveNotifyEvent(G_eventDef.NET_PRODUCTS_RESULT)
...
function ClientScene:StartUpdate(gameinfo)
    ...
    if gameinfo._KindType and gameinfo._KindType == "EasyGame" then
        g_EasyGame:StartFix(gameinfo._KindID,self)
    else
        --更新参数
