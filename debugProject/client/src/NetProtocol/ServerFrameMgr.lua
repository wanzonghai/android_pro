...
function ServerFrameMgr:onSubLogonEvent(sub,pData)
    ...
    elseif sub == G_NetCmd.S_SUB_RELOGON then  --大厅断开，发起重连结果
        dismissNetLoading()
        self:S2C_RelogonResult(pData)
    ...
    elseif sub == G_NetCmd.CUB_MB_SendLogonMobileCode  then             --手机验证码
        local int64 = Integer64:new()
        local result = {
            dwErrorCode = pData:readdword(),        -- 错误码
            tmTimestamp = pData:readscore(int64):getvalue(), -- 当前时间戳（可以不要）
            dwRestSecond = pData:readdword()        -- 剩余时间
        }
        if result.dwErrorCode == 0 then
            print("发送短信成功")
        else
            print("发送短信失败：" .. result.dwErrorCode)
            if result.dwErrorCode == 1010 then
                print("发送得太快了：剩余" .. result.dwRestSecond .. "s")
                showToast(g_language:getString("tel_error_2"))
            end
        end
        G_event:NotifyEvent(G_eventDef.NET_SMS_URL_RESULT,result)
    end
end
...
function ServerFrameMgr:onSubUserEvent(sub,pData)
    ...
    elseif sub == G_NetCmd.S_SUB_USER_FACE_INFO then 
        local cmdtable = g_ExternalFun.readData(logincmd.CMD_GP_UserFaceInfo,pData)
        local wFaceID = cmdtable.wFaceID
        local dwCustomID = cmdtable.dwCustomID
	    GlobalUserItem.wFaceID = wFaceID
        if GlobalUserItem.wFaceID >12 then GlobalUserItem.wFaceID = 12 end          
	    GlobalUserItem.dwCustomID = dwCustomID
        
        G_event:NotifyEventTwo(G_eventDef.NET_MODIFY_FACE_SUCCESS)     
        showToast(g_language:getString("modify_face_success"))   
    ...
    elseif sub == G_NetCmd.SUB_MB_GetProductExtendFlagResult then                --返回：获取商城商品扩展标识
        self:S2C_GetProductExtendFlagResult(pData)
    ...
    elseif sub == G_NetCmd.SUB_MB_GetWithdrawCashLimitResult then                      --返回：获取提现金额限制
        self:S2C_GetWithdrawCashLimitResult(pData)
    ...
    elseif sub ==G_NetCmd.SUB_MB_GetGrowGiftStatusResult then
        --查询VIP信息返回 1505
        self:S2C_GetGrowGiftStatusResult(pData)
    elseif sub ==G_NetCmd.SUB_MB_TakeGrowGiftResult then
        --查询VIP信息返回 1507
        self:S2C_TakeGrowGiftResult(pData)
    elseif sub ==G_NetCmd.SUB_MB_GetGrowLevelConfigResult then
        --查询VIP信息返回 1501
        self:S2C_GetGrowLevelConfigResult(pData)
    ...
    elseif sub == G_NetCmd.SUB_GP_CHECKIN_GET_SERIAL_REWARD_RESULT then 
        --七天连续签到领奖返回
        local cmdtable = g_ExternalFun.readData(logincmd.CMD_MB_SEVEN,pData)
        G_event:NotifyEvent(G_eventDef.SIGN_CONTINUE_RESULT,cmdtable) 
    elseif sub == G_NetCmd.packTest_CMD_MB_ShareLotteryGetSpreadRewardStatus2 then
        --获取奖励信息返回
        local cmdtable = g_ExternalFun.readData(logincmd.CMD_MB_ShareLotteryGetSpreadRewardStatusResult,pData)
        G_event:NotifyEvent(G_eventDef.ShareTurnreceiveInfosReceive,cmdtable) 
    elseif sub == G_NetCmd.packTest_CMD_MB_ShareLotteryTakeSpreadReward2 then
        --领取奖励信息返回
        local cmdtable = g_ExternalFun.readData(logincmd.packTest_CMD_MB_ShareLotteryTakeSpreadReward,pData)
        G_event:NotifyEvent(G_eventDef.ShareTurnreceiveInfosReceiveGet,cmdtable) 
    elseif sub == G_NetCmd.CMD_MB_GETVIP_GIFT_RESULT then  
        --vip对应回扣数据返回
        dismissNetLoading()
        local int64 = Integer64:new()
        local count = pData:getlen() / 8
        local result = {}
        for i=1,count do
            result[i] = pData:readscore(int64):getvalue()
        end  
        GlobalUserItem.share_vip_and_gift = result
        G_event:NotifyEvent(G_eventDef.RECEIVE_VIP_ANDGIFT,pData) 
        G_event:NotifyEvent(G_eventDef.RECEIVE_VIP_ANDGIFT2,pData) 
    elseif sub == G_NetCmd.C_SHARE_PHONE_DATA then
        --分享手机号数据返回
        G_event:NotifyEvent(G_eventDef.EVENT_SHAREPHONE_DATA,pData) 
    elseif sub == G_NetCmd.CMD_GETTIXIAN_LEVTL_RECEIVE then
        --获得提现挡位返回
        G_event:NotifyEvent(G_eventDef.TIXIAN_LEVEL_DATA,pData) 
    elseif sub == G_NetCmd.CMD_SETTIXIAN_LEVTL_RECEIVE then
        --设置提现挡位返回
        G_event:NotifyEvent(G_eventDef.SET_TIXIAN_LEVEL_RECEIVE,pData) 
    elseif sub == G_NetCmd.CMD_TUDO_DENTRO_DATA_RECEIVE then
        --点击全部抽奖返回
        G_event:NotifyEvent(G_eventDef.TUDO_DENTRO_DATA,pData) 
    elseif sub == G_NetCmd.SUB_MB_GetLastWithdrawRecordResult then                --返回：获取商城商品扩展标识
        self:S2C_GetLastWithdrawRecordResult(pData)
    elseif sub == G_NetCmd.CUB_MB_SendBindMobileCode then
        local int64 = Integer64:new()
        local result = {
            dwErrorCode = pData:readdword(),        -- 错误码
            tmTimestamp = pData:readscore(int64):getvalue(), -- 当前时间戳（可以不要）
        ...
        G_event:NotifyEvent(G_eventDef.NET_SMS_URL_RESULT,result)
    elseif sub == G_NetCmd.SMD_REQUEST_BoxInfo then         --拉新分享宝箱返回
        G_event:NotifyEvent(G_eventDef.ShareGiftBoxInfo,pData)
    elseif sub == G_NetCmd.SUB_REQUEST_SHARE_CONFIG then        --拉新配置
        G_event:NotifyEvent(G_eventDef.ShareGift_Config,pData)
    elseif sub == G_NetCmd.SUB_MB_ShareTreasureChestTakeReward then
        G_event:NotifyEvent(G_eventDef.ShareGiftBoxReceive,pData)
    elseif sub == G_NetCmd.SUB_MB_SharePayRebateLoadConfig then     --拉新充值返利配置
        G_event:NotifyEvent(G_eventDef.ShareGetNewBenefitConfig,pData)
    elseif sub == G_NetCmd.SUB_MB_SharePayRebateGetStatus then  --领取拉新宝箱奖励用户状态返回
        G_event:NotifyEvent(G_eventDef.CMD_MB_SharePayRebateGetStatus,pData)
    elseif sub == G_NetCmd.SUB_MB_SharePayRebateGetRecord then
        G_event:NotifyEvent(G_eventDef.CMD_MB_SharePayRebateGetRecord,pData)
    elseif sub == G_NetCmd.SUB_MB_SharePayRebateTakeReward then     --领取拉新奖励返回
        G_event:NotifyEvent(G_eventDef.CMD_MB_SharePayRebateTakeReward,pData)
    end 
end
...
function ServerFrameMgr:getChannelID()      
    ...
    --非官方渠道和网红渠道 拼接上TrackerName 首段    
    if GlobalData.ChannelName == "10001" or GlobalData.ChannelName == " " then
    else
        local pAttribution = g_MultiPlatform:getInstance():getAdjustAttribution() or ""
        if pAttribution ~="" then
            local pAttribution2 = cjson.decode(pAttribution)            
            if pAttribution2 and pAttribution2.trackerName and pAttribution2.trackerName~="" then
                if pAttribution2.trackerName=="Unattributed" then
                    --Facebook 加密安装数据
                    if pAttribution2.fbInstallReferrer then                        
                        local pfbInstallReferrer = cjson.decode(pAttribution2.fbInstallReferrer)
                        if pfbInstallReferrer and pfbInstallReferrer.fb_install_referrer_campaign_group_name and pfbInstallReferrer.fb_install_referrer_campaign_group_name~="" then
                            pfbInstallReferrer.fb_install_referrer_campaign_group_name = string.gsub(pfbInstallReferrer.fb_install_referrer_campaign_group_name,"-","_")
                            pfbInstallReferrer.fb_install_referrer_campaign_group_name = string.upper(pfbInstallReferrer.fb_install_referrer_campaign_group_name)
                            local len = string.len(pfbInstallReferrer.fb_install_referrer_campaign_group_name)
                            local s1 = string.find(pfbInstallReferrer.fb_install_referrer_campaign_group_name,"-") or len
                            local s2 = string.find(pfbInstallReferrer.fb_install_referrer_campaign_group_name,"/") or len
                            local s3 = string.find(pfbInstallReferrer.fb_install_referrer_campaign_group_name,"_") or len
                            local sMin = math.min(s1,s2,s3)
                            sMin = sMin==len and sMin or sMin-1
                            local pName = string.sub(pfbInstallReferrer.fb_install_referrer_campaign_group_name,0,sMin)
                            GlobalData.ChannelName = pName.."-"..GlobalData.ChannelName                        
                        end
                    end                     
                else
                    local len = string.len(pAttribution2.trackerName)                
                    local s1 = string.find(pAttribution2.trackerName,"-") or len
                    local s2 = string.find(pAttribution2.trackerName,"/") or len
                    local s3 = string.find(pAttribution2.trackerName,"_") or len
                    local sMin = math.min(s1,s2,s3)
                    sMin = sMin==len and sMin or sMin-1
                    local pTrackerName = string.sub(pAttribution2.trackerName,0,sMin)
                    pTrackerName = string.trim(string.upper(pTrackerName))
                    if pTrackerName and pTrackerName~="" then
                        GlobalData.ChannelName = pTrackerName.."-"..GlobalData.ChannelName--.."-"..pAttribution2.trackerName
                    end
                end
            end
            EventPost:addCommond(EventPost.eventType.COLLECT,"上报Adjust Attribution 所有数据",nil,nil,pAttribution)                   
        end
    end
    if GlobalData.ChannelName == "" then
        GlobalData.ChannelName = " "
...
function ServerFrameMgr:C2S_LogonByThirdParty(loginType,szAccount,cbgender,szNick,token,email,headurl)
    ...
    printInfo("三方登录",G_NetCmd.MAIN_MB_LOGON,G_NetCmd.C_SUB_LOGON_OTHERPLATFORM)
...
--手机重新登录
function ServerFrameMgr:resetC2S_LogonByPhone(args)
    --机器码
    self:getMachineID()
    ...
    printInfo("手机再登录",G_NetCmd.MAIN_MB_LOGON,G_NetCmd.C_SUB_LOGON_MOBILE2)
	local PhoneData = CCmd_Data:create(185+channelLength)
	PhoneData:setcmdinfo(G_NetCmd.MAIN_MB_LOGON,G_NetCmd.C_SUB_LOGON_MOBILE)
	PhoneData:pushword(G_NetCmd.INVALID_WORD)
	PhoneData:pushdword(self._plazaVersion)
	PhoneData:pushbyte(self._deviceType)
	PhoneData:pushstring(self._szMachine,G_NetLength.LEN_MACHINE_ID)
	PhoneData:pushstring(args.PhoneString,16)
    PhoneData:pushstring(args.szDynamicPass,33)
    for i=1,14 do
        PhoneData:pushbyte(ylAll.ipAddr[i])
    end
    PhoneData:pushutf8(channel)
...
function ServerFrameMgr:C2S_TakeScore(lScore)
    ...
    --2023-05-29去掉银行密码
    -- TakeData:pushstring("8DDCFF3A80F4189CA1C9D4D902C3C909",G_NetLength.LEN_PASSWORD)
    --2023-05-29改成动态密码
    TakeData:pushstring(GlobalUserItem.szDynamicPass,G_NetLength.LEN_PASSWORD)
...
-- - 获取商城商品扩展标识
function ServerFrameMgr:C2S_GetProductExtendFlag()
    local pdata = CCmd_Data:create(4+16*2)
    pdata:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_GetProductExtendFlag)
    pdata:pushdword(GlobalUserItem.dwUserID)
    local productTypeName = 'shop'     -- 商城商品，目前只支持这个
    pdata:pushstring(productTypeName,16*2)   -- 定长32字节
    self:AddNetEvent(pdata)
    return true
end

--返回：商城商品扩展标识
function ServerFrameMgr:S2C_GetProductExtendFlagResult(pData)
    local result = {
        dwCount = pData:readdword(), 
        lsItems = {} 
    }
    
    for i=1, result.dwCount do
        local dwProductID = pData:readdword()
        local dwExtendFlag = pData:readdword()
        result.lsItems[dwProductID] = dwExtendFlag
    end
    dump(result, '获取商城商品扩展标识:' .. pData:getlen() .. 'bytes') 
    G_event:NotifyEvent(G_eventDef.NET_GET_PRODUCT_EXTEND_FLAG,result)  --获取商城商品扩展标识
end
...
function ServerFrameMgr:S2C_GetPayUrlResult(pData)
    ...
    GlobalData.PayURL = pData:readstring(256) --支付地址    
end
...
--请求：获取提现金额限制 3-1782
function ServerFrameMgr:C2S_GetWithdrawCashLimit()
    tlog("ServerFrameMgr:C2S_GetWithdrawCashLimit")
    local pdata = CCmd_Data:create(4)
    pdata:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_GetWithdrawCashLimit)
    pdata:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pdata)
    return true
end
...
--返回：获取提现金额限制
function ServerFrameMgr:S2C_GetWithdrawCashLimitResult(pData)
    tlog("ServerFrameMgr:S2C_GetWithdrawCashLimitResult")
    local len = pData:getlen() 
    if len <= 0 then return end

    local int64 = Integer64:new()
    local result = {
        llMin = pData:readscore(int64):getvalue(),   -- 20 * 100
        llMax = pData:readscore(int64):getvalue()    -- 2000 * 100
    }
    dump(result, '获取提现金额限制:' .. pData:getlen() .. 'bytes')

    G_event:NotifyEvent(G_eventDef.NET_WITHDRAW_CASH_LIMIT_RESULT, result)  --提现信息拉取完成事件
end
...
--获取手机登录验证码
function ServerFrameMgr:C2S_RequestPhoneCode(phoneNumber, smsType)
    phoneNumber = tonumber(phoneNumber)
    local data = CCmd_Data:create(16 + 4 + 1 + 33*2)

    local mainCmd, subCmd
    if smsType == 1 then
        -- 绑定手机
        mainCmd = G_NetCmd.C_SUB_REGISTER_ACCOUNTS
        subCmd = G_NetCmd.SUB_MB_SendBindMobileCode            
    elseif smsType == 2 then
        -- 手机登陆
        mainCmd = G_NetCmd.MAIN_MDM_MB_LOGON
        subCmd = G_NetCmd.SUB_MB_SendLogonMobileCode
    else
        -- other extend smsType, not support now
        return
    end

    -- 命令
    data:setcmdinfo(mainCmd, subCmd)

    -- 参数
    data:pushutf8(phoneNumber,16)
    data:pushdword(smsType == 1 and GlobalUserItem.dwUserID or 0)
    data:pushbyte(smsType)
    data:pushstring(GlobalUserItem.szMachine,33)
    self:AddNetEvent(data)
	return true
end
...
--获取成长等级界面礼包状态 1504
function ServerFrameMgr:C2S_GetGrowGiftStatus()
    showNetLoading()
    local tempData = {
        dwUserID = GlobalUserItem.dwUserID,        
    }
    local pData = g_ExternalFun.writeData(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_GetGrowUserGift,logincmd.CMD_MB_GetGrowUserGift,tempData)
    G_ServerMgr:AddNetEvent(pData)
	return true 
end

--获取成长等级界面礼包状态 返回  1505
function ServerFrameMgr:S2C_GetGrowGiftStatusResult(pData)
    dismissNetLoading()
    --local pData = g_ExternalFun.read_netdata(logincmd.CMD_MB_GetGrowGiftStatusResult,pData)
    local int64 = Integer64:new()
    local result = {
        cbGrowLevel = pData:readbyte(),
        weekGift = {
            cbStatus =  pData:readbyte(),
            llScore =  pData:readscore(int64):getvalue(),
            llNextScore =  pData:readscore(int64):getvalue(),
            llRewardScore =  pData:readscore(int64):getvalue(),
        },
        monthGift =  {
            cbStatus =  pData:readbyte(),
            llScore =  pData:readscore(int64):getvalue(),
    ...
    G_event:NotifyEvent(G_eventDef.EVENT_VIP_GIFT_STATUS,result)
end

--提取成长礼包 发送  1506 1表示周礼金，2表示月礼金, 3表示成长礼金
function ServerFrameMgr:C2S_TakeGrowGift(pType)
    showNetLoading()
    local tempData = {
        dwUserID = GlobalUserItem.dwUserID,    
        szDynamicPass = GlobalUserItem.szDynamicPass,
        szIP = GlobalData.MyIP or "127.0.0.1",
        cbTypeID = pType or 1,
    }
    local pData = g_ExternalFun.writeData(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_TakeGrowGift,logincmd.CMD_MB_TakeGrowGift,tempData)
    G_ServerMgr:AddNetEvent(pData)
	return true 
end

--提取成长礼包 返回 1507
function ServerFrameMgr:S2C_TakeGrowGiftResult(pData)
    dismissNetLoading()
    local result = g_ExternalFun.readData(logincmd.CMD_MB_TakeGrowGiftResult,pData)
    print(result)
    -- GlobalUserItem.VIPInfo = pData
    -- -- GlobalData.VIPEnable = true    
    G_event:NotifyEvent(G_eventDef.EVENT_VIP_TAKE_GIFT_RESULT,result)
end

--请求成长等级配置 1500
function ServerFrameMgr:C2S_GetGrowLevelConfig()
    local goldData = CCmd_Data:create(4)
    goldData:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_GetGrowLevelConfig)
    goldData:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(goldData)
	return true
end
...
--请求成长等级配置 返回 1501
function ServerFrameMgr:S2C_GetGrowLevelConfigResult(pData)
    dismissNetLoading()
    local packLen = pData:getlen()
    local count = packLen / 43
    if count ~= 21 then
        printf("包长不正确，或者协议升级了")
        return;
    else
        printf("包长正确")
    end             
    local child_struct_define = {
        {t = "byte", k = "cbGrowLevel"},            -- 等级
        {t = "score", k = "llBetRequire"},          -- 成长到下一级需求打码量
        {t = "dword", k = "dwPayRequire"},          -- 成长到下一级需求充值量
        {t = "word", k = "wDailyAddition"},         -- 当前等级日转盘加成百分比
        {t = "word", k = "wWeeklyAddition"},        -- 当前等级周转盘加成百分比
        {t = "word", k = "wMonthlyAddition"},       -- 当前等级月转盘加成百分比
        {t = "score", k = "llWeekGift"},            -- 成长周礼包
        {t = "score", k = "llMonthGift"},           -- 成长月礼包
        {t = "score", k = "llGrowGift"},            -- 成长等级礼包
    }
    -- 这里的格式客户自己决定怎么存放，一个长度为21个元素的数组
    local result = {}
    for i = 0 , 20 do
        table.insert(result, g_ExternalFun.readData(child_struct_define, pData) )
    end
    dump(result, '获取成长等级配置:' .. pData:getlen() .. 'bytes')  
    G_event:NotifyEvent(G_eventDef.EVENT_VIP_GET_LEVEL_CONFIG,result)
end
--------------------------------------------------
----游戏相关请求
----
...
--获取推荐奖励信息 20230506
function ServerFrameMgr:getRecommendGiftInfo()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.packTest_CMD_MB_ShareLotteryGetSpreadRewardStatus)
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end

--领取奖励信息 
function ServerFrameMgr:receiveRecommendGift()
    local pack = CCmd_Data:create(4 + G_NetLength.LEN_PASSWORD*2 + 16*2)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.packTest_CMD_MB_ShareLotteryTakeSpreadReward)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushstring(GlobalUserItem.szDynamicPass,G_NetLength.LEN_PASSWORD)
    pack:pushstring(GlobalData.MyIP, 16)
    self:AddNetEvent(pack)
end

--获取vip对应回扣配置
function ServerFrameMgr:receiveVIP_Gift()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_GETVIP_GIFT)
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end

--大厅断开，发起重连
function ServerFrameMgr:C2S_Relogon()
    local pack = CCmd_Data:create(4+33*2+16*2+33*2)
    pack:pushdword(GlobalUserItem.dwGameID)
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalData.MyIP,16)
    pack:pushstring(GlobalUserItem.szMachine,33)
    pack:setcmdinfo(G_NetCmd.MAIN_MB_LOGON, G_NetCmd.C_SUB_RELOGON)
    self:AddNetEvent(pack)
end

--大厅断开，发起重连接收
function ServerFrameMgr:S2C_RelogonResult(pData)
    GlobalData.RelogonTimes = 0
    local cmdtable = g_ExternalFun.readData(logincmd.CMD_MB_Relogon,pData)
    if cmdtable.dwErrorCode == 0 then
        -- 需要保存动态密码到本地
        GlobalUserItem.szDynamicPass = cmdtable.szDynamicPass
    else
        G_event:NotifyEvent(G_eventDef.NET_NEED_RELOGIN)
        -- if cmdtable.dwErrorCode == 100 then
        --     -- 退回到登陆界面        
        -- elseif cmdtable.dwErrorCode == 103 then
        --     -- 便是抢号（同一个账号在多个客户端使用），可退回到登陆界面        
        -- else
        --     -- 退回到登陆界面        
        -- end
    end
...
--请求手机号数据返回
function ServerFrameMgr:requestSharePhoneData()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.S_SHARE_PHONE_DATA)
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end

--获取提现档位 发送 1718
function ServerFrameMgr:requestTiXianLevel()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_GETTIXIAN_LEVTL)
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end

--设置提现档位 发送 1720
function ServerFrameMgr:setTiXianLevel(stageID,stageScore)
    local pack = CCmd_Data:create(4+4+8+33*2+16*2)
    -- 参数
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.CMD_SETTIXIAN_LEVTL)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushdword(stageID)
    pack:pushscore(stageScore)
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalData.MyIP,16)
    self:AddNetEvent(pack)
end
...
--分享抓盘点击全部抽奖
function ServerFrameMgr:TudoDentroBtn(stageScore)
    local pack = CCmd_Data:create(4+2+33*2+16*2)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_TUDO_DENTRO_DATA)
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushword(stageScore)
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalData.MyIP,16)
    self:AddNetEvent(pack)
end
...
-- - 获取最后一笔提现记录
function ServerFrameMgr:C2S_GetLastWithdrawRecord()
    local pdata = CCmd_Data:create(4)
    pdata:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.SUB_MB_GetLastWithdrawRecord)
    pdata:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pdata)
    return true
end
...
--返回：获取最后一笔提现记录
function ServerFrameMgr:S2C_GetLastWithdrawRecordResult(pData)
    local int64 = Integer64:new()
    local result = {
        dwAmount = pData:readdword(), 
        tmSubmitDate = pData:readscore(int64):getvalue(), 
        status = pData:readdword(), 
        szOrderID = pData:readstring(), 
    }
    dump(result, '获取最后一笔提现记录:' .. pData:getlen() .. 'bytes')
    G_event:NotifyEvent(G_eventDef.GetLastWithdrawRecord,result)  --获取商城商品扩展标识
end

--请求宝箱信息
function ServerFrameMgr:requestGiftBoxInfo()
    local pack = CCmd_Data:create(4)
    -- 命令
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_REQUEST_BoxInfo)
    -- 参数
    pack:pushdword(GlobalUserItem.dwUserID)
...
--请求拉新配置信息
function ServerFrameMgr:requestShare_Config()
    local pack = CCmd_Data:create(4)
    -- 命令
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_REQUEST_SHARE_CONFIG)
    -- 参数
    pack:pushdword(GlobalUserItem.dwUserID)
...
--领取拉新宝箱奖励
function ServerFrameMgr:ShareTreasureChestTakeReward(boxIndex,llChestScore)
    local pack = CCmd_Data:create(4+1+8+(33+16+33)*2)
    -- 命令
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_ShareTreasureChestTakeReward)
    -- 参数
    pack:pushdword(GlobalUserItem.dwUserID)
                                
    pack:pushbyte(boxIndex)
    pack:pushscore(llChestScore)                        -- 宝箱奖励的分值,未除以100的原始值
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalData.MyIP,16)
    pack:pushstring(GlobalUserItem.szMachine,33)
...
--获取充值返利配置
function ServerFrameMgr:CMD_MB_SharePayRebateLoadConfig()
    local pack = CCmd_Data:create(4)                 
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_SharePayRebateLoadConfig)
    -- 参数
    pack:pushdword(GlobalUserItem.dwUserID)
    self:AddNetEvent(pack)
end
...
--获取充值返利用户状态
function ServerFrameMgr:CMD_MB_SharePayRebateGetStatus()
    local pack = CCmd_Data:create(4)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE,G_NetCmd.CMD_MB_SharePayRebateGetStatus)
...
--获取充值返利历史记录
function ServerFrameMgr:CMD_MB_SharePayRebateGetRecord(dwPageIndex)
    local pack = CCmd_Data:create(12)             
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_SharePayRebateGetRecord)
             
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushdword(dwPageIndex)
    pack:pushdword(10)
    self:AddNetEvent(pack)
end

--领取返利奖励
function ServerFrameMgr:CMD_MB_SharePayRebateTakeReward(llScore)
    local pack = CCmd_Data:create(4+8+(33+16+33)*2)
    pack:setcmdinfo(G_NetCmd.MAIN_USER_SERVICE, G_NetCmd.CMD_MB_SharePayRebateTakeReward)                                    -- 提取分数,未除以100的原始值
    pack:pushdword(GlobalUserItem.dwUserID)
    pack:pushscore(llScore)
    pack:pushstring(GlobalUserItem.szDynamicPass,33)
    pack:pushstring(GlobalData.MyIP,16)
    pack:pushstring(GlobalUserItem.szMachine,33)    
    self:AddNetEvent(pack)
