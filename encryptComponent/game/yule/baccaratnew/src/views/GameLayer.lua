...
function GameLayer:onEnterRoom()
    self:onExitTable()
    showToast("Como você está apostando há muito tempo, foi solicitado que você saísse da sala, obrigado!")
end
...
function GameLayer:sendSitDown( index, wchair )
    local cmddata = g_ExternalFun.create_netdata(g_var(cmd).CMD_C_OccupySeat)
...
function GameLayer:sendRequestBankInfo()
    local cmddata = g_ExternalFun.create_netdata(g_var(game_cmd).CMD_GR_C_QueryInsureInfoRequest)
...
function GameLayer:onEventGameSceneEnd( dataBuffer )
    ...
    self.m_wCurrentRobApply = suRob
       ...
       self._gameView.m_lUserLastJettonScore = {0,0,0,0,0,0,0,0,0}
    --申请条件
    self._gameView:onGetApplyBankerCondition(cmd_table.lApplyBankerCondition, cmd_table.superbankerConfig)
    ...
    for i=1,g_var(cmd).AREA_MAX do
     ...
     self._gameView.m_lUserLastJettonScore = cmd_table.lUserLastJettonScore[1]
...
function GameLayer:onSocketInsureEvent( sub,dataBuffer )
    ...
    elseif sub == g_var(game_cmd).SUB_GR_USER_INSURE_INFO then --银行资料
        local cmdtable = g_ExternalFun.read_netdata(g_var(game_cmd).CMD_GR_S_UserInsureInfo, dataBuffer)
...
function GameLayer:onSubGameFree( dataBuffer )
    print("game free")
    self.cmd_gamefree = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_GameFree, dataBuffer);
    --游戏倒计时
...
function GameLayer:onSubGameStart( dataBuffer )
    print("game start");
    self.cmd_gamestart = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_GameStart,dataBuffer);
    --播放开始音效
    --g_ExternalFun.playSoundEffect("GAME_START.wav")
    ...
    self._gameView:onGameStart();
     self._gameView.m_lUserLastJettonScore = self.cmd_gamestart.lUserLastJettonScore[1]
end

--用户下注
function GameLayer:onSubPlaceJetton( dataBuffer )
    print("game bet");
    self.cmd_placebet = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_PlaceBet, dataBuffer);
    --g_ExternalFun.playSoundEffect("ADD_SCORE.wav")
    self._gameView:onGetUserBet(self.cmd_placebet);
end
...
function GameLayer:onSubApplyBanker( dataBuffer )
    local cmd_table = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_ApplyBanker,dataBuffer);
...
function GameLayer:onSubChangeBanker( dataBuffer )
    print("change banker")
    local cmd_table = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_ChangeBanker,dataBuffer);
...
function GameLayer:onSubJettonFail( dataBuffer )
    self.cmd_jettonfail = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_PlaceBetFail, dataBuffer)
...
function GameLayer:onSubCancelBanker( dataBuffer )
    print("cancel banker")
    self.cmd_cancelbanker = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_CancelBanker, dataBuffer)

    if self.cmd_cancelbanker.wCancelUser == self.m_wCurrentRobApply then
        self._gameView.m_wCurrentRobApply = G_NetCmd.INVALID_CHAIR
...
function GameLayer:onSubSupperRobBaner( dataBuffer )
    local cmd_table = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_SuperRobBanker, dataBuffer)
...
function GameLayer:onQueryExitGame()
    if PriRoom and true == GlobalUserItem.bPrivateRoom then
        ...
        if self._queryDialog then
          ...
          if self._gameView.m_cbGameStatus == g_var(cmd).GAME_SCENE_FREE then
              --退出防作弊
        ...
        self._queryDialog = QueryDialog:create({"Já começou o jogo, se tiver feito uma aposta o sistema ","deduzirá Tem a certeza que deseja desistir deste jogo?"}, function(ok)
            if ok == true then
                --退出防作弊
                self._gameFrame:setEnterAntiCheatRoom(false)
                print("**************************************3333")
...
function GameLayer:onSubOccupySeat( dataBuffer )
    local cmd_table = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_OccupySeat, dataBuffer)
...
function GameLayer:onSubOccupySeatFail( dataBuffer )
    local cmd_table = g_ExternalFun.read_netdata(g_var(cmd).CMD_S_OccupySeat_Fail, dataBuffer) 
...
function GameLayer:onSubUpdateOccupySeat( dataBuffer )
    ...
    for i = 1, g_var(cmd).MAX_OCCUPY_SEAT_COUNT do
        if cmd_table.tabWOccupySeatChairID[1][i] == G_NetCmd.INVALID_CHAIR then
...
function GameLayer:onUserChat(chat, wChairId)
    ...
    local useritem = self._dataModle:getUserByChair(self._dataModle:getUserList(),wChairId)
