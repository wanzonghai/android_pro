...
function GameResultLayer:showGameResult(selfscore, bankerscore, bankername, bbanker,  UserWinMaxScore,wUserChairID,nMaxUserCount,AllWin ,tabGameResult)
	...
	self.m_selfname = "Próprio"
	self.m_bankername = "Banqueiro"
        ...
        masterCards=	self.m_tabGameResult.masterCards
         ...
         local CardsArray={idleCards,masterCards}
            ...
            local CardsArrayPoint={self.m_tabGameResult.m_cbIdlePoint,self.m_tabGameResult.m_cbMasterPoint}
              ...
              local Image_lose1 = CardPanelArray[1]:getChildByName("Image_lose")
                  local Image_lose2 = CardPanelArray[2]:getChildByName("Image_lose")
                       Image_lose1:setPosition(cc.p(20+80+15,30))
                      Image_lose2:setPosition(cc.p(20+80+15,30))

                      local ldleCount = #CardsArray[1]
                       local MasterCount = #CardsArray[2]
                        Image_lose1:setContentSize(cc.size(ldleCount*30+10,52))
                         Image_lose2:setContentSize(cc.size(MasterCount*30+10,52))
                       if ldleCount>2 then
                            Image_lose1:setPosition(cc.p(20+80+20+10,30))
                            Image_lose1:setContentSize(cc.size(ldleCount*35,52))
                       end
                         if MasterCount>2 then
                            Image_lose2:setPosition(cc.p(20+80+20+10,30))
                              Image_lose2:setContentSize(cc.size(MasterCount*35,52))
                       end
                       
            if CardsArrayPoint[1]>CardsArrayPoint[2] then
                
                    Image_lose2:setVisible(true)
                        Image_lose1:setVisible(false)
               elseif CardsArrayPoint[1]<CardsArrayPoint[2] then
                   
                        Image_lose2:setVisible(false)
                            Image_lose1:setVisible(true)
...
--	self.m_winBankerScore:setString(bankerstr)
   ...
   self.m_selfname = useritem1.szNickName
	...
	local scorestr = g_ExternalFun.formatBetScoreKMBT(self.m_selfscore)
    ...
    if false and self.m_selfscore==0 then
  ...
  --  Text_Name:setString(scorestr)
    ...
    if self.m_bankerscore>=0 then
          Text_Score:setString(bankerstr)
          Text_ScoreFail:setVisible(false)
          Text_Score:setVisible(true)
    else
         Text_ScoreFail:setVisible(true)
          Text_ScoreFail:setString(bankerstr)
          Text_Score:setVisible(false)
    end
	...
	g_ExternalFun.playSoundEffect("GAME_WIN.mp3")
end
...
--	self.m_winBankerScore:setString(bankerstr)
  ...
  --  Text_Name:setString(scorestr)
   ...
   -- Panel_Card:setVisible(false)
	...
	g_ExternalFun.playSoundEffect("GAME_LOSE.mp3")
