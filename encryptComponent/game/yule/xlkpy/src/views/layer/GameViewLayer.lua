...
local HeadSpriteNew = appdf.req(appdf.CLIENT_SRC .. "Tools.HeadSpriteNew")
...
function GameViewLayer:initView(  )
    ...
    bg:setName("bg")
	bg:setPosition(cc.p(ylAll.WIDTH/2,ylAll.HEIGHT/2))
	self._scene.fishBg:addChild(bg, -1)
    if Game_CMD.GAME_PLAYER == 4 then
        if self._scene.m_nChairID < 2 then
           bg:setRotation(180)
           self.particle1:setRotation(180)
           self.particle1:setPosition(cc.p(300, ylAll.HEIGHT))
           self.particle2:setRotation(180)
           self.particle2:setPosition(cc.p(1034, ylAll.HEIGHT))
       end       
    else
   ...
   self:adaptionButton(self.upScore)
	...
	self.upScore:addTouchEventListener(function (sender , eventType)
        ...
        if eventType == ccui.TouchEventType.ended and aaa > 50 then
            ...
            --     end
            --     print("self._scene.ScoreCount = ",self._scene.ScoreCount)
            --     print("self._scene.Copylscore = ",self._scene.Copylscore)
    ...
    self:adaptionButton(self.downScore)
    self.downScore:setVisible(isCloseUpSocre)
		--:setTag(DOWNSCORE)
...
--菜单
    ...
    self:adaptionButton(self.menu)
    self.menu:setTag(TAG.tag_Menu)
    self.menu:setPosition(cc.p(50,120))
    self.menu:addTouchEventListener(function (sender,eventType)
        if eventType == ccui.TouchEventType.ended  then
                self:unHideBtnSchedule()
                self.exit:runAction(cc.MoveTo:create(0.2,cc.p(-50,510)))
                self.sound:runAction(cc.MoveTo:create(0.2,cc.p(-60,400)))
                self.help:runAction(cc.MoveTo:create(0.2,cc.p(-50,290)))
                self.btn_speed:runAction(cc.MoveTo:create(0.2, cc.p(-60,400)))
                --self.byself:runAction(cc.MoveTo:create(0.2,cc.p(1380,450)))  --锁定 连发不runaction
                --self.lock:runAction(cc.MoveTo:create(0.2,cc.p(1380,330)))
                self.upScore:runAction(cc.MoveTo:create(0.2,cc.p(1380,550)))
                self.downScore:runAction(cc.MoveTo:create(0.2,cc.p(1380,230)))
                self.ifmenu = false
    ...
    self.menuT = ccui.Button:create("game_res/show_hide.png")
    self:adaptionButton(self.menuT)
    self.menuT:setTag(TAG.tag_MenuT)
    self.menuT:setVisible(false)
    self.menuT:setRotation(180)
    self.menuT:setPosition(cc.p(50,120))
    self.menuT:addTouchEventListener(function (sender,eventType)
        if eventType == ccui.TouchEventType.ended  then
            self.m_touchTime = currentTime()
            self:hideBtnSchedule()
            self.exit:runAction(cc.MoveTo:create(0.2,cc.p(111,510)))
            self.sound:runAction(cc.MoveTo:create(0.2,cc.p(180,400)))
            self.help:runAction(cc.MoveTo:create(0.2,cc.p(111,290)))
            self.btn_speed:runAction(cc.MoveTo:create(0.2, cc.p(44, 400)))
...
--退出
    ...
    self:adaptionButton(self.exit)
    self.exit:setPosition(cc.p(111,510))
    self.exit:setTag(4)
...
--声音
    ...
    if GlobalUserItem.bSoundAble == false and  GlobalUserItem.bVoiceAble == false then
    ...
    self:adaptionButton(self.btn_speed)
    self.btn_speed:setPosition(cc.p(44, 400))
    ...
    self.btn_speed:addTouchEventListener(function(ref, tType)
        if tType == ccui.TouchEventType.ended then
            ...
            if ref:getTag() == 10 then
                ...
                self._scene.m_cannonLayer.m_bulletSpeed = 0.8		--加速速度      
			elseif ref:getTag() == 20 then
          		--停止特效
				self.speed_effect:stopAllActions()
                self.speed_effect:hide()
                self.btn_speed:setTag(10)
    ...
    self.speed_effect:setPosition(cc.p(self.btn_speed:getContentSize().width / 2 - 6, self.btn_speed:getContentSize().height / 2))
    self.speed_effect:setScale(2.2)
    self.speed_effect:hide()
    self.btn_speed:addChild(self.speed_effect,1)

--帮助
    self.help = ccui.Button:create("game_res/help.png")
    self:adaptionButton(self.help)
    self.help:setPosition(cc.p(111,290))
    self.help:setTag(2)
...
function GameViewLayer:initUserInfo()
    ...
    local score = cc.LabelBMFont:create("", "game_res/scoreNum.fnt")
    ...
    score:setString(g_format:formatNumber(self._scene.m_pUserItem.lScore,g_format.fType.standard,serverKind))
...
function GameViewLayer:updateUserScore( score )
    ...
    if nil ~=  _score then
        ...
        local serverKind = G_GameFrame:getServerKind()
        _score:setString(g_format:formatNumber(score,g_format.fType.standard,serverKind))
    end
end
...
function GameViewLayer:updateMultiple( multiple )
    ...
    if nil ~=  _Multiple then
        ...
        local serverKind = G_GameFrame:getServerKind()
        _Multiple:setString(g_format:formatNumber(multiple,g_format.fType.standard,serverKind))
    end
...
function GameViewLayer:updteBackGround(param, delayTime)
    ...
    local  frame = cc.SpriteFrameCache:getInstance():getSpriteFrameByName("wave_1.png");
    if not frame then
        cc.SpriteFrameCache:getInstance():addSpriteFrames("game_res/wave.plist")
    end
    ...
    groupTips:setPosition(cc.p(ylAll.WIDTH + 180,ylAll.HEIGHT/2))
    ...
    local moveTo1 = cc.MoveTo:create(delayTime, cc.p(180, ylAll.HEIGHT / 2))
    local delayTime1 = (180 + 200) / (1334 / delayTime)
    local moveTo2 = cc.MoveTo:create(delayTime1, cc.p(-200, ylAll.HEIGHT / 2))
        ...
        if nil ~= self._scene and nil ~= self._scene.m_fishLayer then
            ...
            if nil ~= groupTips then
                ...
                for k,v in pairs(self._scene._dataModel.m_fishList) do
                    ...
                    if cc.rectContainsPoint(recWave, fishPos) or not cc.rectContainsPoint(cc.rect(0,0,ylAll.WIDTH,ylAll.HEIGHT), fishPos) then
...
function GameViewLayer:refreshScore( )
    ...
    --dyj1
    local serverKind = G_GameFrame:getServerKind()
    local str = g_format:formatNumber(self._scene.m_pUserItem.lScore,g_format.fType.standard,serverKind)
    --dyj2
    ...
    --银行存款
    local serverKind = G_GameFrame:getServerKind()
    str = g_format:formatNumber(GlobalUserItem.lUserInsure, g_format.fType.standard,serverKind)
    if string.len(str) > 19 then
        str = string.sub(str, 1, 19)
    end

    local serverKind = G_GameFrame:getServerKind()
    self.textBank:setString(g_format:formatNumber(GlobalUserItem.lUserInsure,g_format.fType.standard,serverKind))
...
function GameViewLayer:subMenuEvent( sender , eventType)
    ...
    local function addBG()
        ...
        bg:setContentSize(cc.size(ylAll.WIDTH, ylAll.HEIGHT))
        bg:setScale9Enabled(true)
        bg:setPosition(ylAll.WIDTH/2, ylAll.HEIGHT/2)
    ...
    if 1 == tag then --银行
        ...
        local function sendTakeScore( lScore,szPassword )
            local cmddata = g_ExternalFun.create_netdata(g_var(game_cmd).CMD_GR_C_TakeScoreRequest)
        ...
        local csbNode = g_ExternalFun.loadCSB("game_res/Bank.csb", bg)
        csbNode:setAnchorPoint(0.5,0.5)
        csbNode:setPosition(ylAll.WIDTH/2,ylAll.HEIGHT/2)
...
---------------------------------------------------------------------------------------------------------
    ...
    elseif 2 == tag then --帮助
        ...
        self.helpLayer:addTo(self,2001)
    ...
    elseif 3 == tag then --设置
        ...
        local csbNode = g_ExternalFun.loadCSB("game_res/Setting.csb", bg)
        csbNode:setAnchorPoint(0.5,0.5)
        csbNode:setPosition(ylAll.WIDTH/2,ylAll.HEIGHT/2)
...
function GameViewLayer:ShowCoin( score,wChairID,pos,fishtype )
  ...
  if Game_CMD.GAME_PLAYER == 4 then
     nMyNum = self.m_pUserItem.wChairID >= 2
     nPlayerNum = wChairID >= 2
  end
  ...
  nPos = CannonSprite.getPos(self._scene._dataModel.m_reversal,nPos)
   ...
   --金币类型
    ...
    if nil ~= anim then
        ...
        local num = cc.LabelBMFont:create(g_format:formatNumber(score, g_format.fType.standard,serverKind), "game_res/scoreNum1.fnt")
        ...
        for i=1,coinNum do
            ...
            local tempTime = sqrt((coinPos.x - cannonPos.x) * (coinPos.x - cannonPos.x) + (coinPos.y - cannonPos.y) * (coinPos.y - cannonPos.y)) * 0.003 + random(-10,10) * 0.01
            if tempTime <= 0 then
                tempTime = 0.3
            end
    ...
    if  fishtype ~= nil and fishtype >= Game_CMD.FishType.FishType_JianYu and fishtype <= Game_CMD.FishType.FishType_LiKui then
        ...
        if nil == reward then
            ...
            if goldTxt == nil then
                ...
                goldTxt = cc.LabelBMFont:create(g_format:formatNumber(score, g_format.fType.standard,serverKind), "game_res/scoreNum1.fnt")
    ...
    local bannerText = cc.LabelBMFont:create(g_format:formatNumber(score, g_format.fType.standard,serverKind), "game_res/mutipleNum.fnt")
    ...
    if index >= 3 then
        ...
        if nil ~= text and nil~=text.lScore then
            ...
            local serverKind = G_GameFrame:getServerKind()
            text:setString(g_format:formatNumber(text.lScore,g_format.fType.standard,serverKind))
        end
    else
...
function GameViewLayer:ShowAwardTip(data)
 ...
 local fishName = {
        "Peixe gato amarelo",
        "Carpa de relva pequena",
        "Corvina amarela tropical",
        "Peixe-dourado de olhos grandes",
        "Corvina púrpura tropical",
        "Peixe-palhaço pequeno",
        "Baiacu",
        "Peixe-leão",
        "Peixe lanterna",
        "Tartaruga marinha",
        ...
        "Grande tubarão dourado",
        "Tubarão dourado gigante",
        "Dragão dourado",
        "Balanço da árvore de dinheiro da tartaruga verde milenar",
        "Balanço da árvore de dinheiro da tartaruga prata milenar",
        "Balanço da árvore de dinheiro da tartaruga dourada milenar",
        "Garrafa à deriva",
        "Espada do dragão",
        "Vulcão",
        "Baú do tesouro",
  ...
  if data.nFishMultiple >= 50 then
    if data.nScoreType == Game_CMD.SupplyType.EST_Cold then
       tipStr = "Capturado"..fishName[data.nFishType+1]..",Ganhar"
    elseif data.nScoreType == Game_CMD.SupplyType.EST_Laser then
      
       tipStr = "Use laser e ganhe"
    end

  tipStr1 = string.format("%dMultiple %dFraction",data.nFishMultiple,data.lFishScore)
  if data.nFishMultiple > 500 then
     tipStr2 = "Magnífico!!!"
  elseif data.nFishMultiple == 19 then
       tipStr2 = "Explosão de sorte!!!"   
  else
      tipStr2 = "Super força!!!"     
  end
  ...
  if data.wTableID == self._scene.m_nTableID  then 
    tableStr = "Jogadores da mesa"

  else
       tableStr = string.format("Jogador da mesa %d",data.wTableID+1)
  ...
  table.insert(labelList, lb5)
    ...
    local lb1 =  cc.Label:createWithTTF("Parabéns, você capturou a caixa de suprimentos e ganhou", "fonts/round_body.ttf", 20)
    lb1:setTextColor(cc.YELLOW)
    lb1:setAnchorPoint(0,0.5)

    local lb1 =  cc.Label:createWithTTF(string.format("%dVezes %dPontos !", data.nFishMultiple,data.lFishScore), "fonts/round_body.ttf", 20)
...
function GameViewLayer:ShowTipsForBg( bg )
  ...
  bg:setPosition(ylAll.WIDTH/2, 375)
...
function GameViewLayer:adaptionButton(buttonNode)
    local buttonSize = buttonNode:getContentSize()
    if buttonSize.width > BUTTON_SIZE.width then
        buttonNode:setScale(BUTTON_SIZE.width/buttonSize.width)
    end 
end
