...
function Bullet:setV(ifV)
    if ifV then
       ...
       self.m_moveDir = cc.pForAngle(rad(90-self:getRotation()))
...
function Bullet:normalUpdate( dt )
	...
	if not cc.rectContainsPoint(rect,pos) then
		...
		if pos.x<0 or pos.x>ylAll.WIDTH then
...
function Bullet:followFish(dt)
	...
	self.m_moveDir = cc.pForAngle(rad(90-angle))
...
function Bullet:fallingNet()
	...
	local points2 = {cc.p(0,20),cc.p(-cosf(3.14/6)*20,-sinf(3.14/6)*20),cc.p(cosf(3.14/6)*20,-sinf(3.14/6)*20)} --三个网
...
--发送消息包
	...
	if nil ~= self._gameFrame then
    	...
    	if nil ~= self._gameFrame then
    		self._gameFrame._callBack(-1,"Falha no envio de mensagem de pesca")
    	end
