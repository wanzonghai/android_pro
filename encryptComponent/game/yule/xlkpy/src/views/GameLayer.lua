...
local game_cmd = "client.src.NetProtocol.CMD_GameServer"
...
function GameLayer:ctor(frameEngine, scene)
    ...
    self.m_helpAndroidChairId = -1
    self.m_bLeaveGame = false
    self.m_selfBulletIndex = 0
    self:setReversal()
    self:setPath()
    -- dyj1
...
function GameLayer:addEvent()
    ...
    local function eventListener(event)
        ...
        self.m_cannonLayer:SetSelfBulletIndex(self.m_selfBulletIndex)

        -- 查询本桌其他用户
        self._gameFrame:QueryUserInfo(self.m_nTableID, G_NetCmd.INVALID_CHAIR)
...
function GameLayer:addContact()
    ...
    local function onContactBegin(contact)
        ...
        -- dyj1          碰撞出网清除子弹以及附带数据
        if not self.m_bLeaveGame and nil ~= bullet and tolua.cast(bullet,"cc.Sprite") then
            if nil ~= fish  and tolua.cast(fish,"cc.Sprite") then
...
function GameLayer:AsendCathcFish(bullet, fish)
    ...
    if self._gameFrame == nil then return end
    local cmddata = CCmd_Data:create(18)
    cmddata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_CATCH_FISH);
    ...
    -- 发送失败
    if self.m_bScene and  self._gameFrame and not self._gameFrame:sendSocketData(cmddata) then
        self._gameFrame._callBack(-1, "Falha no envio de mensagem de incêndio")
    end
...
function GameLayer:onCreateSchedule()
    ...
    local function selectMaxFish()
        ...
        if self._dataModel.m_autolock then
            ...
            local rect = cc.rect(0, 0, ylAll.WIDTH, ylAll.HEIGHT)
            local pos = cc.p(fish:getPositionX(), fish:getPositionY())
            local size = fish:getContentSize()
            local fishRect = cc.rect(0,0,size.width,size.height)
            -- 如果鱼在屏幕里
            if not cc.rectContainsPoint(rect, pos) or not cc.rectIntersectsRect(rect,fishRect) then
                self._dataModel.m_fishIndex = self._dataModel:selectMaxFish(true)
            end
        end
        for i=1,4 do
            if self._dataModel.m_helpAndroid_lock[i] then  --帮机器人锁定
                local fish = self._dataModel.m_fishList[self._dataModel.m_helpAndroidFishIndex[i]]
                -- 获取自动锁定的鱼
                if nil == fish then
                    self._dataModel.m_helpAndroidFishIndex[i] = cmd.INT_MAX
                else
                    local rect = cc.rect(0, 0, ylAll.WIDTH, ylAll.HEIGHT)
                    local pos = cc.p(fish:getPositionX(), fish:getPositionY())
                    local size = fish:getContentSize()
...
function GameLayer:Client2ServerFishDismiss(fish)
    if fish and self._gameFrame then
        local cmddata = CCmd_Data:create(10)
        cmddata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_FISH_DEATH)
        cmddata:pushword(self.m_nChairID)
        cmddata:pushint(fish.fish_kind)
...
function GameLayer:Client2ServerHelpSelectLockFish(android_chairId)
     if self._gameFrame == nil then return end
     local cmddata = CCmd_Data:create(8)
     cmddata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_LOCK_FINISH)
     cmddata:pushword(self.m_nChairID)
     cmddata:pushword(android_chairId)
     if self._dataModel.m_helpAndroidFishIndex[android_chairId+1] == Game_CMD.INT_MAX then
        cmddata:pushint(-1)
     else
        cmddata:pushint(self._dataModel.m_helpAndroidFishIndex[android_chairId+1])
     end
     -- 发送失败
     if self._gameFrame and not self._gameFrame:sendSocketData(cmddata) then
         self._gameFrame._callBack(-1, "Falha no envio da mensagem!")
    ...
    for k, m_fish in pairs(self._dataModel.m_fishList) do
        ...
        if m_fish ~= nil and m_fish.m_pathIndex ~= nil and m_fish.m_pathIndex > -1 then
            ...
            if die then
                ...
                --鱼离开，告诉服务端
                self:Client2ServerFishDismiss(m_fish)
        ...
        if self.bullet[i].ifdie then    --子弹死亡
            ...
            if self.bullet[i].m_fishIndex == Game_CMD.INT_MAX then
                ...
                if nil == fish then
                    ...
                    if not cc.rectContainsPoint(cc.rect(0, 0, ylAll.WIDTH, ylAll.HEIGHT), cc.p(fish:getPositionX(), fish:getPositionY())) then
		                self.bullet[i].m_fishIndex = Game_CMD.INT_MAX
                        self.bullet[i]:initPhysicsBody()
                    else
...
function GameLayer:createSecoundSchedule()
    ...
    local function setSecondTips()
        ...
        if nil == self._gameView:getChildByTag(10000) then
            ...
            local time = cc.Label:createWithTTF(string.format("%dSeg", self.m_nSecondCount), "fonts/round_body.ttf", 20)
            ...
            local buttomTip = cc.Label:createWithTTF("60 seg sem disparar, você sairá do jogo logo", "fonts/round_body.ttf", 20)
            ...
            time:setString(string.format("%dSeg", self.m_nSecondCount))
        end
...
function GameLayer:onEnterTransitionFinish()
    ...
    scoredata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_ADDORDOWNSCORE)
    scoredata:pushbool(true)
    --发送失败
    self:sendNetData(scoredata) 
end
...
function GameLayer:onEventUserEnter(wTableID, wChairID, useritem)
    ...
    if wTableID ~= self.m_nTableID or useritem.cbUserStatus == G_NetCmd.US_LOOKON or not self.m_cannonLayer then
        return
    end
...
function GameLayer:onEventUserStatus(useritem, newstatus, oldstatus)
    ...
    if useritem.cbUserStatus == G_NetCmd.US_LOOKON or not self.m_cannonLayer then
        return
    end
...
function GameLayer:enterBackground()
    ...
    if self.m_cannonLayer then
        --self.m_cannonLayer:unAutoSchedule()
        self.m_cannonLayer:setCanShoot(false)
    end
end
...
function GameLayer:onEventGameScene(cbGameStatus, dataBuffer)
    ...
    self._dataModel.m_secene = g_ExternalFun.read_netdata(Game_CMD.GameScene, dataBuffer)
    ...
    self:setUserMultiple()
    self.m_selfBulletIndex = self._dataModel.m_secene.bullet_index or 0
    --设置子弹索引
    if self.m_cannonLayer then
        local index = self._dataModel.m_secene.bullet_index or 0
        self.m_cannonLayer:SetSelfBulletIndex(index)
        self.m_cannonLayer:setCanShoot(true)

        local cannonPos = CannonSprite.getPos(self._dataModel.m_reversal, self.m_nChairID)
        local cannon = self.m_cannonLayer:getCannoByPos(cannonPos + 1)
        if cannon ~= nil then
...
function GameLayer:onEventGameMessage(sub, dataBuffer)
    ...
    elseif sub == Game_CMD.SUB_S_FISH_CREATE then
        ...
        -- dyj1
        local Star = g_ExternalFun.read_netdata(Game_CMD.CMD_S_GameConfig, dataBuffer)
    ...
    elseif sub == Game_CMD.SUB_S_TimeUp and true == PRELOAD.bLoadingFinish then
        -- 限制60s
        showToast("Você foi forçado a sair da sala porque não disparou nada durante um minuto!!!")
...
function GameLayer:onSubCatchFishKingResult(databuffer)
    local data = g_ExternalFun.read_netdata(Game_CMD.CMD_S_CatchSweepFishResult, databuffer)
    for i = 1, 300 do
...
function GameLayer:onSubCatchFishKing(databuffer)
    ...
    local data = g_ExternalFun.read_netdata(Game_CMD.CMD_S_CatchSweepFish, databuffer)
    ...
    elseif fishRealType + 1 == Game_CMD.FishType_ShuiHuZhuan then
        ...
        if data.wChairID == self.m_nChairID then
            ...
            if catchCount > 0 then
                ...
                cmddata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_CATCH_SWEEP_FISH);
                cmddata:pushword(self.m_nChairID)
...
function GameLayer:onSubStopSuperPao(databuffer)
    local data = g_ExternalFun.read_netdata(Game_CMD.CMD_S_BulletIonTimeout, databuffer)
...
function GameLayer:onSubUpdateAllScore(databuffer)
    local data = g_ExternalFun.read_netdata(Game_CMD.CMD_S_UpdateAllScore, databuffer)
...
function GameLayer:onSubSetBulletLimitCount(databuffer)
    local data = g_ExternalFun.read_netdata(Game_CMD.CMD_S_BulletLimitCount, databuffer)
...
function GameLayer:onSubAwardTip(databuffer)
    local award = g_ExternalFun.read_netdata(Game_CMD.CMD_S_AwardTip, databuffer)
...
function GameLayer:onSubBankTake(databuffer)
    local take = g_ExternalFun.read_netdata(Game_CMD.CMD_S_BankTake, databuffer)
...
function GameLayer:onSubShootLaser(databuffer)
    local laser = g_ExternalFun.read_netdata(Game_CMD.CMD_S_Laser, databuffer)
...
function GameLayer:onSubBeginLaser(databuffer)
    local laser = g_ExternalFun.read_netdata(Game_CMD.CMD_S_BeginLaser, databuffer)
    local cannonPos = laser.wChairID
    cannonPos = CannonSprite.getPos(self._dataModel.m_reversal, cannonPos)
    local cannon = self.m_cannonLayer:getCannoByPos(cannonPos + 1)
    if nil ~= cannon then
...
function GameLayer:onSubUpdateFishScore(databuffer)
    local updateFish = g_ExternalFun.read_netdata(Game_CMD.CMD_S_UpdateFishScore, databuffer)
...
function GameLayer:onSubSupplyTip(databuffer)
    ...
    local tip = g_ExternalFun.read_netdata(Game_CMD.CMD_S_SupplyTip, databuffer)

    local tipStr = ""
    if tip.wChairID == self.m_nChairID then
        tipStr = "Ganhou uma caixa de suprimentos! Acerte e você poderá ganhar muitas recompensas! Matem-no agora!"
    else
        ...
        tipStr = userItem.szNickName .. " Ganhou uma caixa de suprimentos! Gostou né, continue firme, talvez você consiga um também!"
    end
...
function GameLayer:onSubMultiple(databuffer)
    ...
    local mutiple = g_ExternalFun.read_netdata(Game_CMD.CMD_S_ExchangeFishScore, databuffer)
    local cannonPos = mutiple.chair_id
    cannonPos = CannonSprite.getPos(self._dataModel.m_reversal, cannonPos)
    local cannon = self.m_cannonLayer:getCannoByPos(cannonPos + 1)
    if mutiple.chair_id == self.m_nChairID then
        if mutiple.swap_fish_score ~= 0 then
            self.ScoreCount = mutiple.swap_fish_score
        end
        if cannon ~= nil then
            self.m_cannonLayer:updateUpScore(self.ScoreCount, cannon.m_pos + 1)
        end
...
function GameLayer:onSubUpdateGame(databuffer)
    local update = g_ExternalFun.read_netdata(Game_CMD.CMD_S_UpdateGame, databuffer)
...
function GameLayer:onSubSupply(databuffer)
    local supply = g_ExternalFun.read_netdata(Game_CMD.CMD_S_Supply, databuffer)
    ...
    if supply.nSupplyType == Game_CMD.SupplyType.EST_Laser then
        if supply.wChairID == self.m_nChairID then
            ...
            tipStr = self.m_pUserItem.szNickName .. "Acertar a caixa de suprimentos dispara um laser! Super arma! Use agora!"
        else
            tipStr = userItem.szNickName .. " Acertar a caixa de suprimentos dispara um laser! Super arma!"
        end

    elseif supply.nSupplyType == Game_CMD.SupplyType.EST_Speed then

        tipStr = userItem.szNickName .. " Acertar a caixa de suprimentos lhe dará aceleração! Dobro da velocidade de todas as balas!"
    elseif supply.nSupplyType == Game_CMD.SupplyType.EST_Null then

        tipStr = "Que pena! Não tem nada na caixa de suprimentos!"
...
function GameLayer:onSubSynchronous(databuffer)
    ...
    local synchronous = g_ExternalFun.read_netdata(Game_CMD.CMD_S_FishFinish, databuffer)
...
function GameLayer:onSubExchangeScene(dataBuffer)
    ...
    local count = floor(dataBuffer:getlen() / 2408)
    if count >= 1 then
        local FishComb = g_ExternalFun.read_netdata(Game_CMD.CMD_S_SwitchScene, dataBuffer)
        -- 鱼阵数据
...
function GameLayer:onSubCreateMoreFishd(databuffer)
    ...
    local count = floor(databuffer:getlen() / 57)
    self.iffishcomb = false
    if count >= 1 then
        for i = 1, count do
            local Fishmore = g_ExternalFun.read_netdata(Game_CMD.CMD_S_FishTrace, databuffer)
...
--            Fishmore.fish_kind = 19
            ...
            if curCount < self.m_fishLimitCount[Fishmore.fish_kind + 1] then
                ...
                fish:setName(Game_CMD.FISH)
                if self._dataModel.m_fishList[fish.fish_id] == nil then
                    self.m_fishLayer:addChild(fish, fish.m_data.fish_kind + 1)
                    self._dataModel.m_fishList[fish.fish_id] = fish
...
function GameLayer:onBackScore(databuffer)
    ...
    local score = g_ExternalFun.read_netdata(Game_CMD.CMD_S_FishMissed, databuffer)
...
function GameLayer:onSubFire(databuffer)
    local fire = g_ExternalFun.read_netdata(Game_CMD.CMD_S_UserFire, databuffer)
    ...
    if fire.android_chairid == self.m_nChairID then
        if fire.is_lock and  (fire.lock_fishid <= 0 or fire.lock_fishid ~= self._dataModel.m_helpAndroidFishIndex[fire.chair_id+1]
        or self._dataModel.m_fishList[fire.lock_fishid] == nil ) then
            local selectNewFish = false
            if fire.lock_fishid == -2 then
                selectNewFish = true
            end
            self._dataModel.m_helpAndroid_lock[fire.chair_id+1] = true
            self._dataModel.m_helpAndroidFishIndex[fire.chair_id+1] = self._dataModel:helpAndroidSelectMaxFish(fire.chair_id+1,selectNewFish)
            self:Client2ServerHelpSelectLockFish(fire.chair_id)
            if selectNewFish == true then  --不进行处理
                return 
            end
        end
        if fire.is_lock == false then
            self._dataModel.m_helpAndroid_lock[fire.chair_id+1] = false
            self._dataModel.m_helpAndroidFishIndex[fire.chair_id+1] = cmd.INT_MAX            
        end
    end
    if not self.m_cannonLayer then
        return
...
function GameLayer:onSubFishCatch(databuffer)
    ...
    local catchNum = floor(databuffer:getlen() / 27)
    --print("======= onSubFishCatch =======", databuffer:getlen())
    if catchNum >= 1 then
        for j = 1, catchNum do
            while true do
                local catchData = g_ExternalFun.read_netdata(Game_CMD.CMD_S_CaptureFish, databuffer)
                if not self.m_cannonLayer then
                    --
                else
...
--                            end
                            ...
                            if fish.m_data ~= nil then
                                self._dataModel:playEffect(string.format("sound_res/samll_%d.mp3", random(6) - 1))
                            end
...
function GameLayer:onSocketInsureEvent(sub, dataBuffer)
    ...
    if sub == g_var(game_cmd).SUB_GR_USER_INSURE_SUCCESS then
        local cmd_table = g_ExternalFun.read_netdata(g_var(game_cmd).CMD_GR_S_UserInsureSuccess, dataBuffer)
    ...
    elseif sub == g_var(game_cmd).SUB_GR_USER_INSURE_FAILURE then
        ...
        local cmd_table = g_ExternalFun.read_netdata(g_var(game_cmd).CMD_GR_S_UserInsureFailure, dataBuffer)
...
function GameLayer:onQueryExitGame()
    ...
    self._queryDialog = QueryDialog:create("Deseja sair do jogo?", function(ok)
        if ok == true then
            self:runAction(cc.Sequence:create(
            cc.DelayTime:create(delayLeaveTime),
            cc.CallFunc:create(
