...
local Clip115Text = appdf.CLIENT_SRC .. "Tools.ClipText"

local cmd = module_pre .. ".models.CMD_Game"
local QueryDialog   = require("client.src.UIManager.QueryDialogNew")
...
function Game1ViewLayer:ctor(scene)
	...
	self:initCsbRes();
    --播放背景音乐
    g_ExternalFun.playBackgroudAudio("xiongdiwushu.mp3")
end
...
function Game1ViewLayer:initCsbRes(  )
	-- rootLayer, self._csbNode = g_ExternalFun.loadRootCSB(GameViewLayer.RES_PATH .."SHZ_Game1Layer.csb", self);
	self._csbNode = g_ExternalFun.loadCSB(GameViewLayer.RES_PATH .."SHZ_Game1Layer.csb", self,false);
    local bg = self._csbNode:getChildByName("game1Bg")	
...
function Game1ViewLayer:initUI( csbNode )
    ...
    --self.m_textScore:setPositionX(self.m_textScore:getPositionX() - 40) 
	--压线
	self.m_textYaxian = csbNode:getChildByName("Text_yaxian");
	self.m_textYaxian:setString(g_var(cmd).YAXIANNUM.."x")
	--压分
	self.m_textYafen = csbNode:getChildByName("Text_yafen");
    --self.m_textYafen:setPositionX(self.m_textYafen:getPositionX() - 25) 
	self.m_textAllyafen = csbNode:getChildByName("Text_allyafen");
    --self.m_textAllyafen:setPositionX(self.m_textAllyafen:getPositionX() - 15) 

	--得到分数
	self.m_textGetScore = csbNode:getChildByName("Text_getscore");
	self.m_textGetScore:setString(g_format:formatNumber(0,g_format.fType.standard,serverKind))

	self.m_textTips = csbNode:getChildByName("Text_Tips")
    --祝您好运
	self.m_textTips:setString("Boa sorte!")
	------
	--菜单  
	self.m_nodeMenu = csbNode:getChildByName("Node_Menu");
    local menuBg = self.m_nodeMenu:getChildByName("Sprite_bg")
    menuBg:setScaleX(1.44)
    self.m_nodeMenu:hide()
	--返回
    ...
    local node = csbNode:getChildByName("Node_2_1")
    node:setOpacity(200)
...
function Game1ViewLayer:game1Begin(  )
    ...
    g_ExternalFun.playSoundEffect("gundong.mp3")
    self:runAction(
    	cc.Sequence:create(
...
function Game1ViewLayer:game1Result()
	...
	--self.m_textGetScore:setString(string.formatNumberCoin(self._scene.m_lGetCoins))
	local serverKind = G_GameFrame:getServerKind()
	self.m_textGetScore:setString(g_format:formatNumber(self._scene.m_lGetCoins,g_format.fType.abbreviation,serverKind))
        					...
        					cc.Sequence:create(
        						...
        						cc.CallFunc:create(function ()
        							if self.m_textTips then
                                        --恭喜中奖
        								self.m_textTips:setString("Parabéns pela vitória!")
        							end
        							...
        							if nodeTip then
                                        ...
                                        winIcon:setPositionX(485)
                                        local serverKind = G_GameFrame:getServerKind()
                                        local strCoin = g_format:formatNumber(self._scene.m_lGetCoins,g_format.fType.abbreviation,serverKind)
                                        local labNum = cc.Label:createWithBMFont("shz_new_fnt_1.fnt", strCoin)
										labNum:setAnchorPoint(cc.p(0,0.5))
										nodeTip:addChild(labNum,0,1)
										local nLen = labNum:getContentSize().width
										labNum:setPosition(650, 375)
    				...
    				cc.CallFunc:create(function()
                        ...
                        --if self._scene.m_bIsAuto == true then
                            ...
                            if self._scene.m_lGetCoins > 0 then
								...
								if self._scene.m_bIsAuto == true then
                                    if self._scene.m_lTotalYafen > self._scene.m_lCoins + self._scene.m_lGetCoins then
                                        --提示游戏币不足
						        	...
						        	if pItem then
						        		...
						        		for j=1,g_var(cmd).ITEM_X_COUNT do
						        			...
						        			if pos.x == posy and pos.y == posx then
						        				...
						        				--显示得分
						        				 self.m_textTips:setString(string.format("Pontuação:%d",pActionOneYaXian.lXianScore))
						        			end
						        		end
...
function Game1ViewLayer:onButtonClickedEvent(tag,ref)
	...
	elseif tag == TAG_ENUM.TAG_MAXADD_BTN  then    --	最大加注
        ...
        --声音
        g_ExternalFun.playSoundEffect("shangfen.mp3")
...
function Game1ViewLayer:onShowTopMenu()
    ...
    self.m_nodeMenu:show()
	self.m_nodeMenu:runAction(spawn)
end
function Game1ViewLayer:onShowMenu()
	local Button_Show = self._csbNode:getChildByName("Button_Show")
	if Button_Show then
...
function Game1ViewLayer:onHelpLayer()
    ...
    self.m_nodeMenu:setPositionX(-667)
    self:onShowMenu()
...
function Game1ViewLayer:setAutoStart( bisShow )
	--显示勾
	local spSelect = self._csbNode:getChildByName("Button_Auto"):getChildByName("game1_check")
	if spSelect then
...
function Game1ViewLayer:updateStartButtonState( bIsStart)
    ...
    if bIsStart == true then
        Button_Start:loadTextureNormal("watermargin-girar.png",UI_TEX_TYPE_PLIST)
    else
        Button_Start:loadTextureNormal("watermargin-pare.png",UI_TEX_TYPE_PLIST)
    end
end
...
function Game2ViewLayer:initCsbRes(  )
	-- rootLayer, self._csbNode = g_ExternalFun.loadRootCSB(GameViewLayer.RES_PATH .. "SHZ_Game2Layer.csb", self);
	self._csbNode = g_ExternalFun.loadCSB(GameViewLayer.RES_PATH .. "SHZ_Game2Layer.csb", self,false);
    local bg = self._csbNode:getChildByName("game2_bg")
    bg:setAnchorPoint(0.5,0.5)
    bg:setPosition(667,375)
    bg:setContentSize(1624,750)
...
function Game2ViewLayer:initUI( csbNode )
	...
	self.m_textScore = csbNode:getChildByName("Text_score");
	local serverKind = G_GameFrame:getServerKind()
	self.m_textScore:setString(g_format:formatNumber(self._scene.m_lCoins,g_format.fType.standard,serverKind))
	--压分
	self.m_textYafen = csbNode:getChildByName("Text_yafen"); 
	local serverKind = G_GameFrame:getServerKind()
	self.m_textYafen:setString(g_format:formatNumber(self._scene.m_lGetCoins,g_format.fType.standard,serverKind))
    self.m_textYafen:setPositionX(self.m_textYafen:getPositionX() - 30)
	--得到分数
	self.m_textGetScore = csbNode:getChildByName("Text_getscore");
...
function Game2ViewLayer:initDiceView(  )
	self._scene:setGame2Mode(1) --GAME2_STATE_WAVING
 	...
 	local dealerAni = cc.Animate:create(cc.AnimationCache:getInstance():getAnimation("dealerDiceAnim"))         
   	...
   	--音效
    g_ExternalFun.playSoundEffect("yaosaizi.mp3")
   	--左边
   	...
   	nodeLight:runAction(
   		cc.Sequence:create(
   			...
   			cc.CallFunc:create(function (  )
   				...
   				--等待音效
                g_ExternalFun.playSoundEffect("xia.mp3")
   	...
   	nodeTouzi:runAction(
   		cc.Sequence:create(
   			...
   			cc.CallFunc:create(function()
                ...
                g_ExternalFun.playSoundEffect(soundStr)
   			end),
   			cc.DelayTime:create(1),
   			cc.CallFunc:create(function()
...
function Game2ViewLayer:initGame2Result()
	...
	if self.m_textGetScore then
		local serverKind = G_GameFrame:getServerKind()
		self.m_textGetScore:setString(g_format:formatNumber(self._scene.m_lAllGetCoin2,g_format.fType.standard,serverKind))
	end
	--展示大骰子
	--创建骰子一
	local spResult1 = self._csbNode:getChildByName("Sprite_result1")
	...
	if self._scene.m_pGame2Result.lScore > 0 then
		--赢的音效
        g_ExternalFun.playSoundEffect("ying.mp3")
	else
		--输的音效
        g_ExternalFun.playSoundEffect("shu.mp3")
	end
  	--默认动作
   	--创建序列帧
...
function Game2ViewLayer:backOneGame()
    ...
    gameview.m_textTips:setString("Boa sorte!")
    gameview.m_textScore:setString(g_format:formatNumber(self._scene.m_lCoins,g_format.fType.standard,serverKind))
...
function Game3ViewLayer:initCsbRes(  )
	self._csbNode = g_ExternalFun.loadCSB(GameViewLayer.RES_PATH .. "SHZ_Game3Layer.csb", self,false);
    local bg = self._csbNode:getChildByName("game3_bg")
    bg:setAnchorPoint(0.5,0.5)
    bg:setPosition(667,375)
    bg:setContentSize(1624,750)
    local Panel = ccui.Layout:create()
...
function Game3ViewLayer:initUI(  )
	...
	--游戏币
    local coinNode = self._csbNode:getChildByName("Text_Coins")
    -- coinNode:setPositionX(coinNode:getPositionX() - 70)
    coinNode:setString("")
    --local pString = string.formatNumberCoin(self._scene.m_lCoins)
    local serverKind = G_GameFrame:getServerKind()
    local pString = g_format:formatNumber(self._scene.m_lCoins,g_format.fType.abbreviation,serverKind)
    local text = cc.Label:createWithBMFont("shz_new_fnt.fnt", pString)
    text:setAnchorPoint(cc.p(0.5, 0.5))
    text:setPosition(0, 0)
    text:addTo(coinNode)
	self.m_textScore = text

	--压分
    coinNode = self._csbNode:getChildByName("Text_On")
	coinNode:setString("")
    -- coinNode:setPositionX(coinNode:getPositionX() + 10)
    --local pString2 = string.formatNumberCoin(self._scene.m_lYafen3)
    local serverKind = G_GameFrame:getServerKind()
    local pString2 = g_format:formatNumber(self._scene.m_lYafen3,g_format.fType.abbreviation,serverKind)
    text = cc.Label:createWithBMFont("shz_new_fnt.fnt", pString2)
    text:setAnchorPoint(cc.p(0.5, 0.5))
    text:setPosition(30, 0)
    text:addTo(coinNode)
    self.m_textYafen = text

	--得到分数
    coinNode = self._csbNode:getChildByName("Text_GetCoins")
	coinNode:setString("")
    -- coinNode:setPositionX(coinNode:getPositionX() - 100)
    text = cc.Label:createWithBMFont("shz_new_fnt.fnt", "0")
    text:setAnchorPoint(cc.p(0.5, 0.5))
    text:setPosition(0, 0)
    text:addTo(coinNode)
    self.m_textGetScore = text
    self.m_textGetScore._curScore = 0

	--得到倍数
	self.m_textTimes = self._csbNode:getChildByName("Text_Times")
    self.m_textTimes:setPositionX(self.m_textTimes:getPositionX() + 15)
			...
			node:runAction(
				cc.Sequence:create(
					cc.CallFunc:create(function (  )
                        ...
                        g_ExternalFun.playSoundEffect("Threegundong.mp3")
					end)
					)
				)
		end
...
function Game3ViewLayer:showPrize( lScore )
    local newScore = self.m_textGetScore._curScore + lScore
    --self.m_textGetScore:setString(string.formatNumberCoin(newScore))
    local serverKind = G_GameFrame:getServerKind()
    self.m_textGetScore:setString(g_format:formatNumber(newScore,g_format.fType.abbreviation,serverKind))
    self.m_textGetScore._curScore = newScore
...
function Game3ViewLayer:backOneGame()
    ...
    --gameview.m_textGetScore:setString(string.formatNumberCoin(self._scene.m_lGetCoins))
    local serverKind = G_GameFrame:getServerKind()
    gameview.m_textGetScore:setString(g_format:formatNumber(self._scene.m_lGetCoins,g_format.fType.abbreviation,serverKind))
