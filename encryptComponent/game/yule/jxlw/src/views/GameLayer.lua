...
function GameLayer:onEventGameSceneFree(buffer)    --空闲 
    ...
    --压分
    --self._gameView.m_textYafen:setString(string.formatNumberCoin(self.m_lBetScore[self.m_bYafenIndex]))
    local serverKind = G_GameFrame:getServerKind()
    local yafen = g_format:formatNumber(self.m_lBetScore[self.m_bYafenIndex],g_format.fType.abbreviation,serverKind)
    self._gameView.m_textYafen:setString(self.m_lYaxian.."x"..yafen) 
    --总压分  Text_allyafen
    local serverKind = G_GameFrame:getServerKind()
    self._gameView.m_textAllyafen:setString(g_format:formatNumber(self.m_lTotalYafen,g_format.fType.standard,serverKind))
    self.m_lJiangjin = cmd_table.lCaiJin
    self._gameView:setJianjin(self.m_lJiangjin)
...
function GameLayer:onAddMaxScore()
    ...
    self.m_lTotalYafen = self.m_lBetScore[self.m_bYafenIndex]*self.m_lYaxian
    --压分
    --self._gameView.m_textYafen:setString(string.formatNumberCoin(self.m_lBetScore[self.m_bYafenIndex] * self.m_lYafen))
    local serverKind = G_GameFrame:getServerKind()
    local yafen = g_format:formatNumber(self.m_lBetScore[self.m_bYafenIndex] * self.m_lYafen,g_format.fType.abbreviation,serverKind)
    self._gameView.m_textYafen:setString(self.m_lYaxian.."x"..yafen)
    --总压分
    local serverKind = G_GameFrame:getServerKind()
    self._gameView.m_textAllyafen:setString(g_format:formatNumber(self.m_lTotalYafen,g_format.fType.standard,serverKind))
