...
function MarqueeMgr:ctor()    
    self.rootNode = {}
    self.defaultPos = cc.p(display.right,display.top*5/6) --默认位置    
    self.dwQueueIndex = 0
...
function MarqueeMgr:initNode()
    if not self.rootNode.csbNode or not tolua.cast(self.rootNode.csbNode,"cc.Node") then
        ...
        if ylAll.ProjectSelect and ylAll.ProjectSelect == 2 then
            -- self.rootNode.mm_ImageCoin:show()
            -- self.rootNode.mm_ImageTC:hide()
            self.rootNode.mm_coin = self.rootNode.mm_ImageCoin
        else
            -- self.rootNode.mm_ImageCoin:hide()
            -- self.rootNode.mm_ImageTC:show()
...
function MarqueeMgr:runAction()
    if #self.m_notifyTable > 0 then
        ...
        self.rootNode.csbNode:setScale(self.ScaleFlag and 0.7 or 1)        
        ...
        performWithDelay(self.rootNode.csbNode,function() 
            ...
            local pMove = cc.MoveTo:create(0.2,cc.p(display.right+1200,display.top*5/6))
            local pDelay = cc.DelayTime:create(delay/1000)
...
function MarqueeMgr:fitDesignResolution(flag)  
    ...
    if self.rootNode.csbNode and not tolua.isnull(self.rootNode.csbNode) then
        ...
        self.rootNode.csbNode:setPositionY(display.top*5/6)
        self.rootNode.csbNode:setScale(flag and 0.7 or 1)
    end
end
