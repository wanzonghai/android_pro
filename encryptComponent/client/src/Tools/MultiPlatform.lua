...
function MultiPlatform:ChangeOrientation(pOrientation)
	local result = false
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).ChangeOrientation then
		result = g_var(PLATFORM[plat]):ChangeOrientation(pOrientation)	
	end
	return result
end
