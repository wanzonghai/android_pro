...
function GlobalUserItem.reSetData()
	...
	GlobalUserItem.RoomList_p                               = {}    --kind*1000 + serverKind*10 + sortID = key  这个列表的key是组合出来的唯一房间标识
	...
	--分享中vip对应的拉取奖励
	GlobalUserItem.share_vip_and_gift = nil
	--服务端标识
	GlobalUserItem.isNewUser								= false			--是否新用户
	GlobalUserItem.isTodayFirstLogin						= false			--是否当日首次登录
	GlobalUserItem.isTester									= false			--是否测试用户
	GlobalUserItem.hasShareWithdraw							= false			--是否有过分享提现操作
end
...
function GlobalUserItem.onLoadData(pData)
	...
	cc.UserDefault:getInstance():setStringForKey("szDynamicPass",tostring(GlobalUserItem.szDynamicPass))
	...
	GlobalUserItem.roomMark = GlobalUserItem.dwLockKindID*1000 + GlobalUserItem.dwLockServerKindID   

	GlobalUserItem.dwAgentID = pData:readdword()     --是否加入公会 非0值就是工会ID
	local pUserFlag = pData:readdword()	
	print("pUserFlag = ",pUserFlag)
	GlobalUserItem.isNewUser								= bit:_and(pUserFlag,1)==1			--是否新用户
	GlobalUserItem.isTodayFirstLogin						= bit:_and(pUserFlag,2)==2			--是否当日首次登录
	GlobalUserItem.isTester									= bit:_and(pUserFlag,4)==4			--是否测试用户
	GlobalUserItem.hasShareWithdraw							= bit:_and(pUserFlag,8)==8			--是否有过分享提现操作
