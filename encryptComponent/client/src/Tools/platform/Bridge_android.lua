...
function Bridge_android.threeLogEvent(ptype,money)  -- ptype 1:"guestLogon" 2:"facebookLogon" 3:"googleLogon" 4:"open" 5:"register" 6:"purchase"
    print("================threeLogEvent")
    local threeData = getThreeDataJson()
    local sigs = "(Ljava/lang/String;)V"   
    local __fire = {"guestLogon","facebookLogon","googleLogon","open","register","purchase"}
    local idx = 1
    if ptype <= 3 then
        idx = 1
    else
    ...
    if threeData.afsdk == true then
        print("========afsdk") 
        local __type = {"af_login","af_open","af_register","af_purchase"}
        local data = {
    ...
    if threeData.adjustsdk == true then
        print("========adjustsdk")
        local __type = {"ad_login","ad_open","ad_register","ad_purchase"}
        local data = {
...
--横竖版切换
function Bridge_android:ChangeOrientation(pOrientation)
    local result = false
    if CHANGE_ORIENTATION_OPEN then
        local sigs = "(I)V"    
        local ok,ret = luaj.callStaticMethod(BridgeClassName.BRIDGE_CLASS_ACTIVITY,FunctionMapping("ChangeOrientation"),{pOrientation},sigs)
        result = ok
    end
    return result
end
