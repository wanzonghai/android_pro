...
if g_TargetPlatform == cc.PLATFORM_OS_ANDROID then
    ...
    BRIDGE_CLASS_ANDROID = "org/cocos2dx/lua/LooqzlGoodGameActivity"
end
...
function WelcomeScene:NewEnterGame(jsondata)
    ...
    if g_TargetPlatform == cc.PLATFORM_OS_ANDROID then
        ...
        if CHANNEL_OPEN then
            ...
            if ok then
                isOfficialChannel = ret == "10001" or ret == " "
            end
        end
        if isOfficialChannel then
...
function  WelcomeScene:EnterClient()
    -- local testScene = require("client/src/testScene")
    -- cc.Director:getInstance():replaceScene(testScene.new())
    -- do return end
