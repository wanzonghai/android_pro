...
function GameFrameEngine:onLogonRoom()
    ...
    if self._roomInfo.wServerKind == G_NetCmd.GAME_KIND_GOLD and GlobalUserItem.lUserScore < self._roomInfo.lEnterScore then
	...
	if self._roomInfo.wServerKind == G_NetCmd.GAME_KIND_TC and GlobalUserItem.lTCCoin < self._roomInfo.lEnterScore then
		local msg = g_language:getString("socre_less_enter_game")
		local str = g_format:formatNumber(self._roomInfo.lEnterScore,g_format.fType.standard,serverKind)
        showToast( string.format(msg,str))
		dismissNetLoading()
