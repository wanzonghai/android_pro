...
function TurnTableLayer:initData()
    ...
    self._platformMinIndex1 = 0
    self._platformMinIndex2 = 0
    self._platformMinIndex3 = 0
    self._userHistoryPageCount = 4
...
function TurnTableLayer:sendServerData()
    G_ServerMgr:C2s_getTurnPlatformNewGifts(4,self["_platformMaxIndex"..self._initLeftIndex],self._initLeftIndex)
end
...
function TurnTableLayer:selectRight(index)
...
function TurnTableLayer:initTurnTableByLeftIndex()
    ...
    for k = 1,#nowConfig do
        ...
        if cbCurrencyType >= 100 then
            ...
            numText:setScale(1)
            numText:setString(self:getValueFormat(g_format:formatNumber(llCurrencyValue,g_format.fType.standard),llCurrencyValue))
        end
        ...
        numText:enableOutline(outlineColor,2)
...
function TurnTableLayer:oldDataList(pData)
    ...
    for k = 1,#oldData do
        ...
        data.llCurrencyValue = data.llCurrencyValue or 0
        data.llAdditionValue = data.llAdditionValue or 0
        type = type or (oldData[1].cbLotteryType + 1)
        if (data.llCurrencyValue + data.llAdditionValue) > 0 then
    ...
    if self._initRightIndex == 1 then
        self:refreshTableView()
        self._tableView:reloadDataInPos()
        self._tableView:scrollToBottom(0.4,false)
    end
end
...
function TurnTableLayer:scrollViewEvent(sender,evenType)
    if evenType == 1 then                                        --滚动到底部
        if self._initRightIndex == 1 then
            if (os.time() - self._findHistoryTime) > 2 then
                self._findHistoryTime = os.time()
                G_ServerMgr:getTurnPlatformHistory(6,self["_platformMinIndex"..self._initLeftIndex],self._initLeftIndex)        --拉取老的记录
            end
        elseif self._initRightIndex == 2 then
            if (not self._userHistoryIsBottom) and (os.time() - self._findHistoryTime) > 2 then
                self._findHistoryTime = os.time()
                G_ServerMgr:getTurnUserHistoryList(self._userHistoryPageCount,self._nowUserPageIndex)
            end
        end
...
function TurnTableLayer:showNodeAnimation(nodePosition,nodeItem,callBack,cbItemIndex)
    ...
    numText:setPosition(cc.p(3,-221))
...
function TurnTableLayer:changeUiByIndex(index)
    if index == self._initLeftIndex then
        return
    end
