...
function HallMessageLayer:ctor(args)
    ...
    self:getMyIP()
    self:showLabel()
    G_event:AddNotifyEvent(G_eventDef.NET_SMS_URL_RESULT, handler(self, self.onSmsUrlResult))
...
function HallMessageLayer:onClickSendMsg()
    ...
    local pTelephone = self.inputTelephone:getString()
    local len = string.len(pTelephone)
    if pTelephone == "" or len ~= 11 then
        showToast(g_language:getString("tel_error_1"))
        return
    end
    --短信验证url
    G_ServerMgr:C2S_RequestSMSUrl()
    self.mm_btn_getCode:setTouchEnabled(false)
end
...
function HallMessageLayer:onBindMobileResultCallback(data)
    if data.dwErrorCode == 0 then 
        self:showAward(data.lRewardScore, "client/res/task/imageNew/mrrw_jb_3.png")       
        self:onClickClose()
