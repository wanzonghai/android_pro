...
function SubLayerJump:onAddEventListen()
   ...
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_GUIDE,handler(self,self.onShowHallGuideLayer))  --绑定成功引导界面 
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_GUIDE_BIND,handler(self,self.onShowHallGuideBind))  --短信验证引导页 
   ...
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_HALL_VIP_REWARD,handler(self,self.onShowVIPRewardLayer))  --VIP详情界面
   ...
   G_event:AddNotifyEvent(G_eventDef.CHANGE_HEAD,handler(self,self.onChangeHead))                  --改变头像
   G_event:AddNotifyEvent(G_eventDef.shareVipDescrible,handler(self,self.onShowShareVipDescrible))    --分享推荐奖励vip对应关系
   G_event:AddNotifyEvent(G_eventDef.showSHAREPHONEDATA,handler(self,self.onShowSharePhoneLayer))    --分享手机号返回

   G_event:AddNotifyEvent(G_eventDef.UI_OPEN_COMMON_DIALOG,handler(self,self.onShowCommonDialog))    --大厅通用提示框
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_SELECTLAYER,handler(self,self.onShowSelectLayer))     --选择挡位界面
   G_event:AddNotifyEvent(G_eventDef.UI_OPEN_RECHARGECODE,handler(self,self.onShowRechargeQrCode))  --商城扫码支付
   G_event:AddNotifyEvent(G_eventDef.UI_SHOW_PG_LIST,handler(self,self.onShowHallPGLayer))             --打开PG游戏列表
end

function SubLayerJump:onRemoveListen()
   G_event:RemoveNotifyEvent(G_eventDef.UI_OPEN_RECHARGECODE)  
    G_event:RemoveNotifyEvent(G_eventDef.showSHAREPHONEDATA)  
    G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_SELECTLAYER)  
    G_event:RemoveNotifyEvent(G_eventDef.shareVipDescrible)  --分享推荐奖励页面
    G_event:RemoveNotifyEvent(G_eventDef.CASH_TIPSLAYER)  --提现提示
    G_event:RemoveNotifyEvent(G_eventDef.CHANGE_HEAD)    --头像改变
   ...
   G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_GIFT_CODE_SHOP)   --激活码限时礼包商城界面
    ...
    G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_HALL_VIP_REWARD)    
    ...
    G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_GUIDE)
    G_event:RemoveNotifyEvent(G_eventDef.UI_SHOW_GUIDE_BIND)
    G_event:RemoveNotifyEvent(G_eventDef.UI_OPEN_COMMON_DIALOG)
end

--改变头像界面
function SubLayerJump:onChangeHead(args)
   appdf.req(luaPath.ChangeHeadLayer).new(args)
end
...
--绑定引导界面
function SubLayerJump:onShowHallGuideLayer(args)
   appdf.req(luaPath.hallGuideLayer).new(args)
end

--短信验证引导页
function SubLayerJump:onShowHallGuideBind(args)
   appdf.req(luaPath.hallGuideBind).new(args)
end
...
--VIP领奖界面
function SubLayerJump:onShowVIPRewardLayer(args)
   appdf.req(luaPath.hallVIPRewardLayer).new(args)
end
...
--分享奖励vip对应关系
function SubLayerJump:onShowShareVipDescrible(args)
   appdf.req(luaPath.ShareVipDescrible).new(args)  
end

--分享手机号数据界面
function SubLayerJump:onShowSharePhoneLayer(args)
   appdf.req(luaPath.SharePhoneLayer).new(args)  
end

--大厅通用提示框
function SubLayerJump:onShowCommonDialog(args)
   local parent = cc.Director:getInstance():getRunningScene()
   if args.name then
      local pLayer = parent:getChildByName(args.name)
      if pLayer then
         return
      end
   end
   appdf.req(luaPath.CommonDialog).new(args)  
end

--选择挡位界面
function SubLayerJump:onShowSelectLayer(args)
   appdf.req(luaPath.ShareSelectLayer).new(args)
end

--商城支付界面
function SubLayerJump:onShowRechargeQrCode(args)
   appdf.req(luaPath.RechargeQrCodeLayer).new(args)
end

--打开PG游戏列表
function SubLayerJump:onShowHallPGLayer(args)
   appdf.req(luaPath.HallPGLayer).new(args)
end
