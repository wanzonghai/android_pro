...
--EasyGame
g_EasyGame = appdf.req(appdf.CLIENT_SRC.."ExtraGame.EasyGame")
--PocketGames
g_PocketGame = appdf.req(appdf.CLIENT_SRC.."ExtraGame.PocketGame")
...
function LogonScene:onEnterTransitionFinish()    
    --上报AD应用打开事件
    g_MultiPlatform:getInstance():threeLogEvent(4)
    g_ExternalFun.stopMusic()-- 暂停音乐 
...
function LogonScene:ctor(scene,needBg)   
    ...
    --EasyGame 同步远端版本 加载本地版本
    g_EasyGame:LoadVersion() 
...
function LogonScene:DealAutoLogin()
    if ALARM_NOTIFICATION_OPEN then
        --支持定时通知        
        -- local pSmall = string.format(ylAll.SERVER_UPDATE_DATA.NotificationURL,1,1)
        -- local pBig = string.format(ylAll.SERVER_UPDATE_DATA.NotificationURL,1,2)
        g_MultiPlatform:getInstance():PushImageNotification("","")
    ...
    elseif GlobalUserItem.LoginType == 1 then           --手机号登录
        local phoneNumber = cc.UserDefault:getInstance():getStringForKey("TELEPHONE","")
        local szDynamicPass = cc.UserDefault:getInstance():getStringForKey("szDynamicPass","")
        if phoneNumber and #phoneNumber > 0 and #szDynamicPass > 0 then
            self:reOnPhoneLogin({PhoneString = phoneNumber,szDynamicPass = szDynamicPass})
        else
            self:onOpenPhoneLogin()
        end
    end
...
--手机重新登录  
function LogonScene:reOnPhoneLogin(pData)
    self:ShowloadingBar()
    self:CheckLoginTimeout()
    self.LoginData = {1}
    G_ServerMgr:checkConnectStatus()
    G_ServerMgr:resetC2S_LogonByPhone(pData)
end
...
function LogonScene:onThirdAuthCallback(args)
    ...
    local pConnectStatus = G_ServerMgr:checkConnectStatus()
    print("pConnectStatus = ",pConnectStatus)
...
function LogonScene:LogonSuccess(args)    
   ...
   -- self.PanelTips
    ...
    if GlobalUserItem.isNewUser then
        --上报AD注册事件
        g_MultiPlatform:getInstance():threeLogEvent(5)
    end
end
