		...
		local function createArray(pTargetT,pLenT,pLen,pCall)
			...
			if pLen > #pLenT then
		 	...
		 	for i=1,pLenT[pLen] do
		  		if #pLenT == pLen then
		  			table.insert(pTargetT,pCall());
		  		else
		  			local t = {};
		  			table.insert(pTargetT,t);
	...
	for k,v in pairs(keyTable) do
		...
		elseif "tchar" == keyType then
			...
			elseif keys.ss then  --需要自己先读取大小的
				keyFun = function()
					local __strLen = dataBuffer:readint();
					if __strLen == 0 then
						return "";
					end
					return dataBuffer:readstring(__strLen);
				end
			else
				keyFun = function() return  dataBuffer:readstring(); end
			end
		elseif "char" == keyType then
			if keys.ss then
				keyFun = function()
					local __strLen = dataBuffer:readword();
					if __strLen == 0 then
						return "";
					end
					return dataBuffer:readutf8(__strLen);
				end
			elseif keys.s then
				if keys.s == 0 then
					keyFun = function() 
						local ss =   dataBuffer:readutf8(); 
						return  ss; 
					end
				else
					keyFun = function() 
						local ss = dataBuffer:readutf8(keys.s); 
						return  ss; 
					end
				end
			end
		...
		elseif "table" == keyType then
			keyFun = function()
				return ExternalFun.readData(keys["d"],dataBuffer)
			end
		elseif "double" == keyType then
			keyFun = function() return  dataBuffer:readdouble(); end
		elseif "float" == keyType then
		...
		if nil ~= keyFun then
			cmd_table[key] = ExternalFun.read_datahelper1({strkey = key, lentable = lenT, fun = keyFun});
		end
	end
	return cmd_table;
end
...
--挂载脚本到子节点中去
function ExternalFun.addScriptForChildNode(node,ScriptFileFath,tab)
...
local function CheckChinese(s) 
	local ret = {};
    local f = '[%z\1-\9\11-\127\194-\244][\128-\191]*';
...
function ExternalFun.getRoomMark(kindID,serverKind,sortID)
	return kindID*100 + serverKind*10 + sortID	
end
--获取kind
function ExternalFun.getKindID(roomMark)
    return math.modf(roomMark/100)
end
--获取 serverKind
function ExternalFun.getServerKind(roomMark)
	local remainder = math.fmod(roomMark,100)
