...
redPointMgr.eventType = {
    ...
    shareTurn       = 8,        --分享转盘
    shareTurn_1     = 0x801, 
    shareTurn_2     = 0x802,    --分享转盘可提现
    shareTurn_3     = 0x803,    --分享转盘有返利可领取
    vip             = 9,        --vip红点 
    vipSub_1        = 0x901,    --是否有成长礼包可领取
    vipSub_2        = 0x902,  
    vipSub_3        = 0x903,  
}
...
function redPointMgr:onChangeRedPointCallback(data)
    ...
    for k, v in pairs(data.lsItems) do
        ...
        -- print("bySubType = ",bySubType)
        if self.saveRedTable and self.saveRedTable[v.byType] and self.saveRedTable[v.byType][bySubType] then
