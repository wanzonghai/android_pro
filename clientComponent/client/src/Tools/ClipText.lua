...
ClipText.emClipType = g_ExternalFun.declarEnumWithTable(0, emClipType);
------
...
function ClipText:onExit(  )
	...
	g_ExternalFun.SAFE_RETAIN(self.m_actMoveAct);
end
...
function ClipText:updateMoveAction( text )
	g_ExternalFun.SAFE_RELEASE(self.m_actMoveAct);
	local m_szVisibleSize = self.m_szVisibleSize;
