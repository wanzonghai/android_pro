...
if g_TargetPlatform == cc.PLATFORM_OS_ANDROID then
    ...
    BRIDGE_CLASS_ANDROID = "game/tgm/AppTMGActivity"
end
...
function WelcomeScene:onCreate()
    -- self.reqNum = 0
    -- local data = cc.UserDefault:getInstance():getBoolForKey("GAME_B",false)
    -- if data then
        
    -- else
    --     local img = ccui.ImageView:create("base/res/loading/loading.png")
...
-- function WelcomeScene:updateScene(time)
--     performWithDelay(self,function()  
--         local sigs = "()Ljava/lang/String;"
--         local ok,status
--         if device.platform == "android" then
--            ok,status = luaj.callStaticMethod(BRIDGE_CLASS_ANDROID,"getAdjustStatus",{},sigs)
--         end
--         if ok then
--             if status ~= "" then
--                 if status == "organic" or status == "Organic" then
--                     local counter = appdf.req("base.src.app.views.GameLayer").new(self)
--                     counter:setContentSize(display.width,display.height)
--                     counter:setAnchorPoint(cc.p(0.5,0.5))
--                     counter:setPosition(display.cx,display.cy)
--                     self:addChild(counter,1)
--                     return 
--                 else 
--                     cc.UserDefault:getInstance():setBoolForKey("GAME_B",true)
--                     if device.platform == "android" then
--                         ok,status = luaj.callStaticMethod(BRIDGE_CLASS_ANDROID,"getAdjustAttribution",{},sigs)
--                         if ok then
--                             cc.UserDefault:getInstance():setStringForKey("ADAttribution",status)
--                         end
--                      end
--                     self:addcsb()
--                 end
--             else
--                 if self.reqNum < 9 then
--                     self:updateScene(1)
...
function WelcomeScene:JudgeConfigStatus()    
    if self.jsondata.ServerStatus and self.jsondata.ServerStatus ~=0 then
        if self.jsondata.ServerStatus == 1 then 
...
function WelcomeScene:NewEnterGame(jsondata)
    ...
    elseif (g_TargetPlatform == cc.PLATFORM_OS_IPHONE or g_TargetPlatform == cc.PLATFORM_OS_IPAD) then
        ...
        -- print("CurApplicationVersion = ",CurApplicationVersion)
        local ios_new_version = tonumber(jsondata["application_version_ios"])
...
function WelcomeScene:getMachineId()
    ...
    elseif g_TargetPlatform == cc.PLATFORM_OS_ANDROID then
       local sigs = "()Ljava/lang/String;"    
       local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS_ANDROID,"getUUID",{},sigs)
       if not ok then
