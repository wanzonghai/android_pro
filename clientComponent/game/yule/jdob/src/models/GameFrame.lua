...
function GameFrame:updateUserBetScore(_chairId, _betScore)
    local hasCurPlayer = false
    for i, v in ipairs(self.m_userBetScoreArray) do
        if v.chairId == _chairId then
            hasCurPlayer = true
            v.betScore = v.betScore + _betScore
            break
        end
    end
    tlog('GameFrame:updateUserBetScore hasCurPlayer ', hasCurPlayer, _chairId, _betScore)
    if not hasCurPlayer then
        local data = {}
        data.chairId = _chairId
        data.betScore = _betScore
...
function GameFrame:getAllUserBetScore()
    tlog("jdob_GameFrame:getAllUserBetScore")
