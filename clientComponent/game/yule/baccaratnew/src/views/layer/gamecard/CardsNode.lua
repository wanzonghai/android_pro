...
function CardsNode:ctor()
    csbCardNode =   cc.CSLoader:createNode("game/Layer.csb");
  csbCardNode:addTo(self)
   --self:addToRootLayer(csbCardNode, ZORDER_LAYER.ZORDER_HAND_Layer)
     ...
     local action = cc.CSLoader:createTimeline('game/Layer.csb')
...
function CardsNode:updateCardsNode( cards, isShowCard, bAnimation, pCallBack)
	...
	local centerPos = cc.p(winSize.width * 0.5-200, winSize.height * 0.5-100);
...
function CardsNode:addCards(cbCard, pCallBack)
	...
	--百家乐，顶部位置
	local winSize = cc.Director:getInstance():getWinSize()
	local centerPos = cc.p(winSize.width * 0.5-300, winSize.height-200)
	centerPos = self:convertToNodeSpace(centerPos)
	...
	if showAnimation then
		...
		for  i=1,count do
			...
			if tmp:getDispatched() then
				...
				local delay = cc.DelayTime:create(ANI_BEGIN*0.1);
            
               tmp:runAction(cc.ScaleTo:create(ANI_BEGIN*0.5, 0.45)) 
               --   tmp:runAction(cc.ScaleTo:create(ANI_BEGIN*0.5+ANI_RATE(idx), 0.45)) 
				...
				local spawn = cc.Spawn:create(moveTo2, cc.CallFunc:create(function()
                   ...
                   -- moveTo2
				--[[local runSeq = cc.Sequence:create(cc.DelayTime:create(ANI_RATE(idx)), spawn, cc.DelayTime:create(0.05),cc.CallFunc:create(function() 
                       ...
                       --[[ if Tag==1 then
                         ...
                         -- moveTo2
				local runSeq = cc.Sequence:create(cc.DelayTime:create(ANI_RATE(idx)), moveTo2,cc.CallFunc:create(function() 
