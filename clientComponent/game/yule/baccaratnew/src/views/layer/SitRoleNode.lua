...
function SitRoleNode:ctor( viewParent, index )
	local function nodeEvent( event )
		if event == "exit" then
			g_ExternalFun.SAFE_RELEASE(self.m_actShowScore)
		end
	end
	...
	--加载csb资源
	local csbNode = g_ExternalFun.loadCSB("game/SitRoleNode.csb", self)
	...
	g_ExternalFun.SAFE_RETAIN(self.m_actShowScore)
...
function SitRoleNode:onSitDown( useritem, bAni, isBanker )
	...
	local head = PopupInfoHead:createClipHead(useritem, self.m_headSize, "game/hlssm_head_mask.png")
	if nil ~= head then
		self.m_csbNode:addChild(head)
       head:setPosition(cc.p(0,14))
		...
		head:enableInfoPop(false, pos, anchor)
	end	
	...
	--游戏币
	local str = g_ExternalFun.numberThousands(useritem.lScore)
...
function SitRoleNode:gameEndScoreChange( useritem, changescore )
	...
	local str = "." .. g_ExternalFun.numberThousands(changescore)
