...
function GamePlayerList:ctor(parent,data)
    ...
    self.playlist = data
    local csbNode = g_ExternalFun.loadCSB("game/yule/baccarat/res/PlayListLayer.csb")
	...
	self._tableView = cc.TableView:create(cc.size(1060,564))
...
function GamePlayerList:tableCellAtIndex( view, idx )
    ...
    local item = cell:getChildByName(idx)
    if not item then
        item = g_ExternalFun.loadCSB("game/yule/baccarat/res/nodePlayer.csb",nil,false)
        item:setTag(idx)
        cell:addChild(item)
        item:setPosition(530,56)
    end
    for i=1,2 do
