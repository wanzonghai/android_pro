...
local racordPath = {
    "GUI/record/record_green.png",
    "GUI/record/record_yellow.png",
    "GUI/record/record_red.png"
}
...
function GameViewLayer:updateRecord(color, winData)
    ...
    local img = self:createRocerdItem(data)
...
function GameViewLayer:upRecordList()
    ...
    for i, v in ipairs(self.m_recordData) do
        local img = nil
        if i <= itemCount then
            img = self.m_recordMaxItems[i]
            img:setTexture(racordPath[v.color + 1])
            img:getChildByName("recordText"):setString(v.winData)
        else
            img = self:createRocerdItem(v)
            self.mm_record_list:addChild(img)
            table.insert(self.m_recordMaxItems,img)
        end
        local imgSize = img:getContentSize()
