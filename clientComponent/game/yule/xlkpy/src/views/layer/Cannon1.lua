...
function Cannon:ctor(viewParent)
	...
	self.m_index  = 0 --子弹索引
	self.m_ChairID  = G_NetCmd.INVALID_CHAIR
...
function Cannon:initWithUser(userItem)
    ...
    self.m_nickName = userItem.szNickName
...
function Cannon:shootLaser()
	...
	local moveDir = cc.pForAngle( rad(90-angle))
	cc.pMul(moveDir,50)
	...
	if  self.m_ChairID == self.m_nChairID then 
        ...
        cmddata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_LASER)
        cmddata:pushfloat(self.m_fort:getRotation())

        self.power = 0

		if not self.frameEngine:sendSocketData(cmddata) then
			self.frameEngine._callBack(-1,"Falha no envio de mensagem de laser")
		end
	end
end
...
function Cannon:productBullet( isSelf,fishIndex, netColor,fire)
    ...
    if fire ~= nil then
        ...
        if self.parent.parent.m_bScene  and  self.frameEngine then
	...
	if isSelf then
		...
		local cmddata = CCmd_Data:create(29)
   		cmddata:setcmdinfo(G_NetCmd.MAIN_GAME, Game_CMD.SUB_C_FIRE)
    	cmddata:pushint(self.m_index)
        cmddata:pushint(self.m_Type)
    	cmddata:pushbyte(1)
        local final_angle =(deg(pi/2 -angle))/57.32
        if Game_CMD.GAME_PLAYER == 4 then
            if self.m_nChairID <2 then
        ...
        cmddata:pushfloat(self.parent.m_bulletSpeed)
         ...
         if self.parent.parent.m_bScene  and  self.frameEngine then
		    if not self.frameEngine:sendSocketData(cmddata) then
			    self.frameEngine._callBack(-1,"Falha no envio de mensagem de incêndio")
            else
...
function Cannon:setCannonType( cannontype,times)	
	...
	if cannontype == Game_CMD.CannonType.Special_Cannon then
        ...
        --]]
		Type:runAction(cc.RepeatForever:create(CirCleByManager:create(1.0,cc.p(pos.x,pos.y),10)))
...
function Cannon:ShowSupply( data )
	...
	if nSupplyType == Game_CMD.SupplyType.EST_Laser then
		...
		if self.m_ChairID == self.m_nChairID then
			...
			ptPos.x = random(200) + 200
			ptPos.y = random(200) + 200
		end
...
function Cannon:autoUpdate(dt)
	...
	if score < self.parent.parent.CurrShoot[1][self.m_nChairID+1] then
		...
		if nil == self._queryDialog then
      		local this = self
	    	self._queryDialog = QueryDialog:create("Marque primeiro!", function(ok)
	        this._queryDialog = nil
	    	end)
            local scene = cc.Director:getInstance():getRunningScene()
            scene:addChild(self._queryDialog)
    	end
	...
	if self.m_autoShoot and  self._dataModel.m_autolock  then
		...
		if nil ~= frame then
			...
			if nil == sp then
				...
				sp:runAction(cc.RepeatForever:create(CirCleByManager:create(1.0,cc.p(pos.x,pos.y),10)))
			else
		...
		if self._dataModel.m_reversal then
			pos = cc.p(ylAll.WIDTH-pos.x,ylAll.HEIGHT-pos.y)
		end
        ...
        local endAngle = angle - self:getRotation()
        if endAngle>=180 and endAngle<=210 then
           endAngle = 210
        elseif endAngle>=150 and endAngle<=180 then
            endAngle = 150
        end
		self.m_fort:setRotation(endAngle)
...
function Cannon:lockBubble1(dt)
	...
	local function update(dt)
        ...
        if (fish ~= nil and not self.m_other and self.m_autoShoot) or (fish ~= nil and self.m_otherLock and self.m_other) then
            ...
            if cc.rectContainsPoint(cc.rect(0, 0, ylAll.WIDTH, ylAll.HEIGHT), fishPos) and cc.rectIntersectsRect(rect,fishRect)  then
                local mePos = cc.p(self:getPositionX(), self:getPositionY())
                local angle = self.m_fort:getRotation()
            
                local distance = cc.pGetDistance(fishPos, mePos)
                local num = floor(distance / 50)
                for i = 1, num do
...
function Cannon:otherUpdate(dt)
	...
	if self.m_cannonPoint.x == 0 and self.m_cannonPoint.y == 0 then 
        local x,y = self.m_fort:getPosition()
    ...
    if Game_CMD.GAME_PLAYER == 4 then
       if self.m_nChairID>1 then
          if fire.chair_id>1 then            --3,4同边位置
             ifchange=true
          else
             ifchange=false
          end
       else
          if fire.chair_id<2 then            --1,2同边位置
             ifchange=true
          else
             ifchange=false
          end
    ...
    if fire.lock_fishid ~= Game_CMD.INT_MAX then
        ...
        if nil ~= fish then
            ...
            if cc.rectContainsPoint(cc.rect(0, 0, ylAll.WIDTH, ylAll.HEIGHT), fishPos) then
