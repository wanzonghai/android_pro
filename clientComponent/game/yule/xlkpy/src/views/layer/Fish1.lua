...
--			local bzierpoint = bezier
		...
		point = cc.p(point.x+ random(0,50),point.y+ random(0,50))
        --dyj2
		local m_oldPoint = cc.p(self:getPositionX(),self:getPositionY())
		self:setConvertPoint(point)


		if cc.rectContainsPoint( cc.rect(0,0,ylAll.WIDTH, ylAll.HEIGHT), point ) then
...
function Fish:RotatePoint(pcircle,dradian,ptsome)
	...
	tmp.x = ptsome.x * cosf(dradian) - ptsome.y * sinf(dradian) + pcircle.x
	tmp.y = ptsome.x * sinf(dradian) + ptsome.y * cosf(dradian) + pcircle.y
...
function Fish:initAnim()
	...
	--宝箱
    if self.m_data.fish_kind == Game_CMD.FishType_BaoXiang then
        local tips = "tips/tips_" .. self.m_data.fish_kind .. ".png"
        local tips_score = "tips/tips_score_" .. self.m_data.fish_kind .. ".png"
        self._scene._gameView:Showtips1(tips, tips_score)        
    end
   ...
   -- self.bomType=self.m_data.m_fudaifishtype
    ...
    if self.m_pathIndex == 48 and self.nType < 10 then                              --限制条件8
        self.Xpos=Game_CMD.PathIndex[self.m_pathIndex][1][1] - 180 * cosf( rad(12*self.fortieth))
        self.Ypos=Game_CMD.PathIndex[self.m_pathIndex][1][2] + 180 * sinf( rad(12*self.fortieth))
