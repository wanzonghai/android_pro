...
local fishTraceLua = {
module_pre .. ".models.fishTrace1_1",
module_pre .. ".models.fishTrace1_2",
module_pre .. ".models.fishTrace1_3",
module_pre .. ".models.fishTrace1_4",
module_pre .. ".models.fishTrace1_5",

module_pre .. ".models.fishTrace2_1",
module_pre .. ".models.fishTrace2_2",

module_pre .. ".models.fishTrace3_1",
module_pre .. ".models.fishTrace3_2",
module_pre .. ".models.fishTrace3_3",
module_pre .. ".models.fishTrace3_4",
module_pre .. ".models.fishTrace3_5",
module_pre .. ".models.fishTrace3_6",

module_pre .. ".models.fishTrace4_1",
module_pre .. ".models.fishTrace4_2",

module_pre .. ".models.fishTrace5_1",
module_pre .. ".models.fishTrace5_2",
module_pre .. ".models.fishTrace5_3",
module_pre .. ".models.fishTrace5_4",
module_pre .. ".models.fishTrace5_5"
}
...
function PreLoading.PerLoadingLuaRes(callback)
    PreLoading._processTotal = 20
	PreLoading._processIndex = 1
    PreLoading._processComplete = callback
    PreLoading._processTimer = tickMgr:delayedCall(PreLoading.doLoadLuaTrace, 1000 / 30, -1);
end

function PreLoading.doLoadLuaTrace()
    if PreLoading._processTotal < PreLoading._processIndex then
		if PreLoading._processTimer ~= nil then
		   PreLoading._processTimer:destroy()
		   PreLoading._processTimer = nil
		end
		applyFunction(PreLoading._processComplete)
		PreLoading._processComplete = nil
    else
        appdf.req(fishTraceLua[PreLoading._processIndex])

        PreLoading.loadingPer = 30 + PreLoading._processIndex*3.3
        PreLoading.updatePercent(PreLoading.loadingPer)

        PreLoading._processIndex = PreLoading._processIndex + 1
    end
end
...
function PreLoading.loadTextures()
    ...
    local function loadFishTrace()
		...
		if PreLoading.bFishData then
			...
			layer:stopAllActions()
			layer:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),callfunc))
		end
        PreLoading.Finish()
        --print("资源加载完成")
    end
	...
	local function imageLoaded(texture)
        ...
        if m_nImageOffset == totalSource then
            ...
            --开始加载鱼阵路径
            if userNewLoadType then
                PreLoading.PerLoadingLuaRes(function() loadFishTrace() end)
            else
                loadFishTrace()
            end
        end
...
function PreLoading.GameLoadingView()
	...
	loadingBG:setPosition(cc.p(ylAll.WIDTH/2,ylAll.HEIGHT/2))
	...
	loadingBarBG:setPosition(cc.p(ylAll.WIDTH/2,ylAll.HEIGHT/2-300))
	layer:addChild(loadingBarBG)
	--]]

	local loading_text = ccui.ImageView:create("loading/loading_text.png")
	loading_text:setTag(4)
	loading_text:setPosition(cc.p(ylAll.WIDTH/2,ylAll.HEIGHT/2 - 205))
...
function PreLoading.updatePercent(percent )
	...
	if nil ~= PreLoading.loadingBar then
        ...
        PreLoading.loadingBar:setPercentage(percent)
		--cc.Director:getInstance():getActionManager():removeActionByTag(1, PreLoading.fish)
		--local move =  cc.MoveBy:create(1,cc.p(1060*(percent/100),PreLoading.loadingBar:getContentSize().height/2))
		--move:setTag(1)
		PreLoading.fish:setPositionX(1060*(percent/100))
	end
