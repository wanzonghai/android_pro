...
local res_path = "game/yule/jxlw/res/"
...
function PreLoading.loadTextures()
	...
	local plists = {"game1/gameAction/dagu.plist",
					...
					"game1/tiger-hall-package.plist",
        ...
        if m_nImageOffset == totalSource then
			...
			if PreLoading.bFishData  then
				...
				local callfunc1 = cc.CallFunc:create(function()
					PreLoading.loadingBG:loadTexture(res_path.."loading/preBg_01.png")
				end)
			...
			--	layer:runAction(cc.Sequence:create(callfunc1,cc.DelayTime:create(0.8),callfunc2,cc.DelayTime:create(0.8),callfunc3,cc.DelayTime:create(0.8),callfunc4))
				layer:runAction(cc.Sequence:create(cc.DelayTime:create(0.8),cc.DelayTime:create(0.8),callfunc4))
			end
        	print("资源加载完成")
        end
    end
...
function PreLoading.GameLoadingView()
	...
	-- local menu_node = cc.CSLoader:createNode(res_path.."SHZ_Loading.csb")
	-- --menu_node:setScale(self._scale_factor)

	-- local actor = cc.CSLoader:createTimeline(res_path.."SHZ_Loading.csb")
	-- menu_node:runAction(actor)
	-- actor:gotoFrameAndPlay(0,true)
	-- layer:addChild(menu_node)
	...
	PreLoading.loadingBG:setPosition(cc.p(ylAll.WIDTH/2,ylAll.HEIGHT/2))
	...
	loadingBarBG:setPosition(cc.p(ylAll.WIDTH/2,12))
