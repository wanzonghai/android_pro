...
function Game1ViewLayer:initUI( csbNode )
	...
	--压分
    local yafenNode = csbNode:getChildByName("Text_yafen")
    yafenNode:setString("")
    ...
    local oldYafenNode = csbNode:getChildByName("Text_allyafen");
	...
	--得到分数
    local oldGetNode = csbNode:getChildByName("Text_getscore");
...
function Game1ViewLayer:game1Begin()
    --cc.Director:getInstance():setAnimationInterval(1/5)
    --ExternalFun.playSoundEffect("fs_gundong.mp3")
    --self._txtHashId:setString(self._scene.m_dwHashID)
    self.m_textTips:setVisible(false)
