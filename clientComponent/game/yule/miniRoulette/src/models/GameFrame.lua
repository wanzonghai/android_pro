...
function GameFrame:updateUser(userItem)
    ...
    if userItem.wChairID ~= G_NetCmd.INVALID_CHAIR then
        self.m_tableChairUserList[userItem.wChairID + 1] = userItem
        self.m_tableUidUserList[userItem.dwUserID] = userItem       
    end
...
--获取玩家下注前10名的信息
function GameFrame:getTopTenUserBetScore(_betArea)
    tlog('GameFrame:getTopTenUserBetScore ', _betArea)
    local betScoreArray = {}
    for i, v in ipairs(self.m_userBetScoreArray) do
        local betScore = v.betScore[_betArea]
        if betScore > 0 then
            local data = {}
            data.chairId = v.chairId
            data.betScore = betScore
            table.insert(betScoreArray, data)
        end
    end
    tdump(betScoreArray, "betScoreArray", 10)
    table.sort(betScoreArray, function (first, second)
        return first.betScore > second.betScore
    end)

    local finialArray = {}
    local curCount = 0
    for i, v in ipairs(betScoreArray) do
        local userInfo = self.m_tableChairUserList[v.chairId + 1]
        if userInfo then
            v.userName = userInfo.szNickName ----插入名字
            if curCount < 10 then
                table.insert(finialArray, v)
                curCount = curCount + 1
            else
                break
            end
...
function GameFrame:getAllUserBetScore()
    tlog("double_GameFrame:getAllUserBetScore")
