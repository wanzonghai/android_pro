...
function DoubleGoldLayer:initGold(goldNum)
	for j = 1, goldNum do
        local sp = cc.Sprite:create("GUI/double_bet_icon_1.png")
		sp:setVisible(false)
...
function DoubleGoldLayer:getScoreSprite(betScore, _isSelf)
	...
	coinSprite:setTexture(string.format("GUI/double_bet_icon_%d.png", _betIndex))
				...
				for j = 1, value do
					...
					if curUseItemNum <= allItemGoldNum then
						...
						if spriteJetton then
							spriteJetton:setTexture(string.format("GUI/double_bet_icon_%d.png", k))
						...
						for m = 1, needNum do
							...
							if sprite then
								sprite:setTexture(string.format("GUI/double_bet_icon_%d.png", k))
