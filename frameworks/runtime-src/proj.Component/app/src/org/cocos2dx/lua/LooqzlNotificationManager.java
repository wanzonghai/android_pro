...
import kbrrugm.ghza.mqihm.ahiosklz.R;
...
public class LooqzlNotificationManager extends BroadcastReceiver {
    public static String TAG = "MyNotificationManager";
    protected static NotificationCallback mNotificationCallback;
    protected static LooqzlNotificationManager mUnityNotificationManager;
    ...
    public LooqzlNotificationManager() {
        super();
    }

    public LooqzlNotificationManager(Context context, Activity activity) {
        super();
        ...
        try {
            ...
            if (rescheduleOnRestart) {
                ComponentName receiver = new ComponentName(activity, LooqzlNotificationRestartOnBootReceiver.class);
            ...
            mOpenActivity = LooqzlNotificationUtilities.getOpenAppActivity(context, false);
    ...
    public static LooqzlNotificationManager getNotificationManagerImpl(Context context) {
        return getNotificationManagerImpl(context, (Activity) context);
    }

    // Called from managed code.
    public static LooqzlNotificationManager getNotificationManagerImpl(Context context, Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            mUnityNotificationManager = new LooqzlNotificationManagerOreo(context, activity);
        } else {
            mUnityNotificationManager = new LooqzlNotificationManager(context, activity);
        }
    ...
    public void setNotificationCallback(NotificationCallback notificationCallback) {
        LooqzlNotificationManager.mNotificationCallback = notificationCallback;
    }
    ...
    protected static NotificationChannelWrapper getNotificationChannel(Context context, String id) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return LooqzlNotificationManagerOreo.getOreoNotificationChannel(context, id);
        }
    ...
    protected NotificationChannelWrapper getNotificationChannel(String id) {
        return LooqzlNotificationManager.getNotificationChannel(mContext, id);
    }
    ...
    public static void scheduleNotificationIntent(Intent data_intent_source) {
        ...
        String temp = LooqzlNotificationUtilities.serializeNotificationIntent(data_intent_source);
        Intent data_intent = LooqzlNotificationUtilities.deserializeNotificationIntent(mContext, temp);

        int id = data_intent.getIntExtra("id", 0);

        Intent openAppIntent = LooqzlNotificationManager.buildOpenAppIntent(data_intent, mContext, mOpenActivity);
        ...
        if (intent != null) {
            if (mRescheduleOnRestart) {
                LooqzlNotificationManager.saveNotificationIntent(mContext, data_intent);
            }

            PendingIntent broadcast = getBroadcastPendingIntent(mContext, id, intent, PendingIntent.FLAG_UPDATE_CURRENT);
            LooqzlNotificationManager.scheduleNotificationIntentAlarm(mContext, intent, broadcast);
        }
    }
    ...
    protected static void saveNotificationIntent(Context context, Intent intent) {
        ...
        SharedPreferences.Editor editor = prefs.edit().clear();
        String data = LooqzlNotificationUtilities.serializeNotificationIntent(intent);
        ...
        LooqzlNotificationManager.loadNotificationIntents(context);
    }
    ...
    protected static List<Intent> loadNotificationIntents(Context context) {
        ...
        for (String id : ids) {
            ...
            if (serializedIntentData.length() > 1) {
                Intent intent = LooqzlNotificationUtilities.deserializeNotificationIntent(context, serializedIntentData);
        ...
        for (String id : idsMarkedForRemoval) {
            LooqzlNotificationManager.deleteExpiredNotificationIntent(context, id);
        }
    ...
    public void cancelPendingNotificationIntent(int id) {
        LooqzlNotificationManager.cancelPendingNotificationIntent(mContext, id);
        if (this.mRescheduleOnRestart) {
            LooqzlNotificationManager.deleteExpiredNotificationIntent(mContext, Integer.toString(id));
        }
    }

    // Cancel a pending notification by id.
    protected static void cancelPendingNotificationIntent(Context context, int id) {
        Intent intent = new Intent(context, LooqzlNotificationManager.class);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            ...
            LooqzlNotificationManager.sendNotification(context, intent);
    ...
    protected static void sendNotification(Context context, Intent intent) {
        Notification.Builder notificationBuilder = LooqzlNotificationManager.buildNotification(context, intent);
        int id = intent.getIntExtra("id", -1);

        LooqzlNotificationManager.notify(context, id, notificationBuilder.build(), intent);
    }
    ...
    protected static Notification.Builder buildNotification(Context context, Intent intent) {
        ...
        notificationBuilder.setSmallIcon(R.drawable.abc_fly_ic_notification);
...
//            mNotificationCallback.onSendNotification(intent);
            try{
                ...
                NotificationCallback m_notification = (NotificationCallback)Class.forName("org.cocos2dx.lua.LooqzlNotificationCallback")
                        .newInstance();
        ...
        if (!isRepeatable)
            LooqzlNotificationManager.deleteExpiredNotificationIntent(context, Integer.toString(id));
    }
}
