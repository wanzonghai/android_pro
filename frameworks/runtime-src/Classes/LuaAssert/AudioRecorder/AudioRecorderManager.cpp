...
static const char *JAVA_CLASS = "game/tgm/AppActivity";
#endif
...
static AudioRecorderManager *_recorderInstance = nullptr;
AudioRecorderManager::AudioRecorderManager() : m_bInit(false)
{
...
AudioRecorderManager *AudioRecorderManager::getInstance()
{
    ...
    {
        _recorderInstance = new AudioRecorderManager();
    }
    return _recorderInstance;
}

void AudioRecorderManager::destroy()
{
...
void AudioRecorderManager::init(const std::string &recordPath, const std::string &downloadPath)
{
    m_bInit = true;
...
void AudioRecorderManager::startRecord(const std::string &filename /*= "record.mp3"*/)
{
    if (false == m_bInit)
    {
        log(" do not init AudioRecorderManager ");
        return;
    }
...
void AudioRecorderManager::endRecord()
{
    if (false == m_bInit)
    {
        log(" do not init AudioRecorderManager ");
...
void AudioRecorderManager::cancelRecord()
{
    if (false == m_bInit)
    {
        log(" do not init AudioRecorderManager ");
...
static int toLua_AudioRecorder_getInstance(lua_State *tolua_S)
{
    ...
    if (1 == argc)
    {
        AudioRecorderManager *ret = AudioRecorderManager::getInstance();
        object_to_luaval<AudioRecorderManager>(tolua_S, "cc.AudioRecorderManager", (AudioRecorderManager *)ret);
        return 1;
    }
    return 0;
}
...
{
    AudioRecorderManager *ret = (AudioRecorderManager *)tolua_tousertype(tolua_S, 1, nullptr);
    if (nullptr != ret)
    {
        int argc = lua_gettop(tolua_S);
        if (argc == 3)
    ...
    log(" init AudioRecorderManager error ");
    return 0;
}
    ...
    if (nullptr != ret)
    {
        ...
        if (FileUtils::getInstance()->isFileExist(fileFullPath))
        {
            ...
            if (0 != size)
            {
                ...
                buffer->WriteUInt32((uint32_t *)&cmd.dwTargetUserID);
                buffer->WriteUInt32((uint32_t *)&size);
                buffer->WriteUInt08(cmd.byVoiceData, size);
                // buffer->PushByteDataNHJ((BYTE*)&cmd.dwTargetUserID, 4);
                // buffer->PushByteDataNHJ((BYTE*)&size, 4);
                // buffer->PushByteDataNHJ((BYTE*)&cmd.byVoiceData, size);
    ...
    if (nullptr != ret)
    {
        ...
        if (argc == 3)
        {
            ...
            if (nullptr != buffer)
            {
                ...
                if (idx + 4 > buffer->GetBufferLenghtqw())
                {
                    return 0;
                }
                // CMD_GF_S_UserVoice
                ...
                buffer->ReadUInt32((uint32_t *)&dwSendUerId);
                ...
                buffer->ReadUInt32((uint32_t *)&dwTargetUserID);
                // idx = buffer->GetCurrentIndex();
                // if (idx + 4 > buffer->GetBufferLenghtqw())
                //{
                //     return 0;
                ...
                buffer->ReadUInt32((uint32_t *)&dwVoiceLength);

                // idx = buffer->GetCurrentIndex();
                // if (idx + dwVoiceLength > buffer->GetBufferLenghtqw())
                //{
                //     return 0;
                // }
                ...
                if (fp)
                {
                    fwrite(buffer->GetBufferLave(), sizeof(unsigned char), dwVoiceLength, fp);
                    fclose(fp);
                }
...
int register_all_recorder()
{
    ...
    tolua_usertype(tolua_S, "cc.AudioRecorderManager");
    tolua_cclass(tolua_S, "AudioRecorderManager", "cc.AudioRecorderManager", "", nullptr);

    tolua_beginmodule(tolua_S, "AudioRecorderManager");
    ...
    std::string typeName = typeid(AudioRecorderManager).name();
    g_luaType[typeName] = "cc.AudioRecorderManager";
    g_typeCast["AudioRecorderManager"] = "cc.AudioRecorderManager";
#endif
    return 1;
}