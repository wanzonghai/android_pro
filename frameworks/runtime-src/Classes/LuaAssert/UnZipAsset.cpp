...
CUnZipAssetManger::CUnZipAssetManger(const char* szFilePath,const char* szUnZipPath,int nHandler)
...
CUnZipAssetManger::~CUnZipAssetManger()
{

}

void CUnZipAssetManger::UnZipFile(const char* szFilePath,const char* szUnZipPath,int nHandler)
{
	CUnZipAssetManger *pUnzipAsset = new CUnZipAssetManger(szFilePath,szUnZipPath,nHandler);
	pUnzipAsset->autorelease();
	pUnzipAsset->retain();
    std::thread thr(&CUnZipAssetManger::UnZipRun, pUnzipAsset);
    thr.detach();
}

void CUnZipAssetManger::UnZipRun()
{
     //解压
    int result = 0;
     //创建解压
    ...
    if(m_nHandler != 0)
    {
        Global_PushSyncData(MSG_UN_ZIP, m_nHandler, result, 0);
