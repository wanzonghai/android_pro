...
//构造函数
CImageToByteManger::CImageToByteManger() :m_pData(NULL), m_nWidth(0), m_nHeight(0)
{

}
//析构函数
CImageToByteManger::~CImageToByteManger()
{
	CC_SAFE_DELETE(m_pData);
}
//加载数据
bool CImageToByteManger::onLoadData(const char* respath)
{
	bool result = false;
	...
	do{
		if (!m_szRes.empty())
		{
			...
			{
				CCLOG("[CImageToByteManger] has init %s", respath);
				break;
			}
		}
		...
		{
			CCLOG("[_DEBUG] CImageToByteManger error width-%d height-%d", m_nWidth, m_nHeight);
			break;
		}
...
//获取数据
unsigned int CImageToByteManger::getData(int x, int y)
{
	if (m_szRes.empty())
		return 0;
...
//删除数据
void CImageToByteManger::onCleanData()
{
	m_szRes = "";
	CC_SAFE_DELETE(m_pData);
}